...
namespace SE::BuildTool
{
    ...
    static void BuildBindingsHeaderInfoFromTypes(TypeDatabase const& database, HeaderInfo const& headerInfo, std::vector<TypeInfoBase*> & typesInHeader, BindingsHeaderInfo& outInfo)
    {
        ...
        database.GetInjectCodeFormHead(headerInfo.headerId, outInfo.injectedCode);
    ...
    bool Generator::Generate(TypeDatabase const& database, SolutionInfo const& solution)
    {
        ...
        for ( auto& prj : solution.projects)
        {
            ...
            GenerateModuleCodeFile(database, *pProjectDesc, typesInProject, expectedFiles);
    ...
    void Generator::GenerateModuleCodeFile(TypeDatabase const& database,
                                           ...
                                           std::vector<std::string> const& expectedFiles)
    {
        ...
        std::vector<std::string> autoGeneratedFiles = expectedFiles;
        std::sort(autoGeneratedFiles.begin(), autoGeneratedFiles.end());
        autoGeneratedFiles.erase(std::unique(autoGeneratedFiles.begin(), autoGeneratedFiles.end()), autoGeneratedFiles.end());
