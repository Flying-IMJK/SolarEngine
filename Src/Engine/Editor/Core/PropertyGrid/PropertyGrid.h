...
namespace SE::Editor
{
    ...
    public:
        ...
        IType*                         m_pOwnerTypeInstance = nullptr; // The type being edited by the property grid
        IType*                         m_pTypeInstanceEdited = nullptr; // The internal type instance that we modified - this is generally gonna be the same the owner type instance
        TypeProperty const*         m_pPropertyInfo = nullptr; // The info about the property that we edited (the property info refers to the typeInstanceEdited)
    ...
    public:
        ...
        PropertyGrid( EditorContext const* pEditorContext );
        ~PropertyGrid();
        ...
        IType const* GetEditedType() const { return m_pTypeInstance; }

        // Get the current edited type
        IType* GetEditedType() { return m_pTypeInstance; }

        // Set the type instance to edit, will reset dirty status
        void SetTypeToEdit( IType* pTypeInstance );
    ...
    private:
        ...
        TypeCompositeInfo const*                                    m_pTypeInfo = nullptr;
        IType*                                             m_pTypeInstance = nullptr;
        bool                                                        m_isDirty = false;
        bool                                                        m_isControlBarVisible = true;
        bool                                                        m_showReadOnlyProperties = false;
        bool                                                        m_isReadOnly = false;
        #ifdef SE_DEVELOPMENT
        GUI::FilterWidget                                           m_filterWidget;
        #endif

        List<PG::CategoryRow*>                                      m_categories;
...
namespace SE::Editor::PG
{
    ...
    public:
		...
		List<GridRow*> const& GetChildren() const { return m_children; }
        ...
        inline void RecursiveOperation( Function<void(GridRow*)> function)
        {
            function( this );
    ...
    public:
        ...
        static CategoryRow* FindOrCreateCategory( GridRow* pParentRow, GridContext const& context, List<CategoryRow*>& categories, String const& categoryName );

    public:

        CategoryRow( GridRow* pParentRow, GridContext const& context, String const& name );

        void AddProperty( IType* pTypeInstance, TypeProperty const& propertyInfo );
    ...
    public:
        ...
        ArrayRow( GridRow* pParentRow, GridContext const& context, TypeProperty const& propertyInfo, IType* pParentTypeInstance );
    ...
    public:
        ...
        PropertyRow( GridRow* pParentRow, GridContext const& context, TypeProperty const& propertyInfo, IType* pParentTypeInstance, int32_t arrayElementIndex = -1 );
        ~PropertyRow();
    ...
    private:
        ...
        TypeProperty const&     m_propertyInfo;
        IType*                  m_pParentTypeInstance = nullptr; // Our direct parent type instance
        int32                            m_arrayElementIdx = -1; // If we are an array element this will be set
