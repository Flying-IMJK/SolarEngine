...
namespace THelpers
{
    template <typename T, bool IsTriviallyDestructible = std::is_enum<T>()>
...
//    enum { Value = TOrValue<__is_pod(T) || __is_enum(T), TIsPointer<T>>::Value };
	enum { Value = TOrValue<std::is_pod_v<T> || std::is_enum_v<T>, TIsPointer<T>>::Value };
};
...
struct TIsBaseOfT
{
    enum { Value = std::is_base_of<Base, Derived>::type };
};
...
// 检查是否是 lambda 函数
template <typename T>
struct TIsLambda : std::integral_constant<bool, std::is_class_v<T> && !std::is_default_constructible_v<T>>
{
...
struct LambdaTraits<Lambda, std::void_t<decltype(&Lambda::operator())>> {
    using CallOperator = decltype(&Lambda::operator()); // 提取 operator() 的类型
    using Signature = typename LambdaTraits<CallOperator>::Signature;
};

// 特化：提取成员函数指针的签名
template<typename T, typename ReturnType, typename... Params>
struct LambdaTraits<ReturnType(T::*)(Params...) const> {
    using Signature = ReturnType(Params...);
};
...
//    enum { Value = __is_constructible(T, typename TAddLValueReference<typename TAddConst<T>::Type>::Type) };
	enum { Value = std::is_constructible<T, typename TAddLValueReference<typename TAddConst<T>::Type>::Type>::Value };
...
{
	enum { Value = TOrValue<std::is_trivially_copyable<T>::value, TIsPODType<T>>::Value };
//#if defined(__clang__) && __clang_major__ >= 15
...
{
	enum { Value = TOrValue<std::is_trivially_assignable<T, const T>::value, TIsPODType<T>>::Value };
//#if defined(__clang__) && __clang_major__ >= 15
...
inline to* DynamicCast(from* v) noexcept
{
	static_assert(TIsBaseOf<from, to>::Value || TIsBaseOf<to, from>::Value, "Can not cast an pointer.");
