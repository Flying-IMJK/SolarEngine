...
namespace SE
{
	...
	public:
		...
		explicit Matrix3x3(float values[9])
		{
		...
		explicit Matrix3x3(float values[3][3])
		{
			Platform::MemoryCopy(Raw, values, sizeof(float) * 9);
		}
	...
	public:
		...
		Float3 GetColumn(int column) const
		{
			return {Raw[column], Raw[4 + column], Raw[8 + column]};
		}
	...
	public:
		...
		static void Scaling(float scale, Matrix3x3& result)
		{
			Scaling(scale, scale, scale, result);;
		}
...
template<>
struct TIsPODType<SE::Matrix3x3>
{
	enum { Value = true };
};

DEFINE_DEFAULT_FORMATTING(SE::Matrix3x3, "[M11:{0} M12:{1} M13:{2}] [M21:{3} M22:{5} M23:{5}] [M31:{6} M32:{7} M33:{8}]", v.M11, v.M12, v.M13, v.M21, v.M22, v.M23, v.M31, v.M32, v.M33);
