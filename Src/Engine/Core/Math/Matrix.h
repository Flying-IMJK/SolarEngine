...
namespace SE
{
	...
	public:
		...
		explicit Matrix(float values[4][4])
		{
			Platform::MemoryCopy(Raw, values, sizeof(float) * 16);
		}
	...
	public:
		...
		Float4 GetColumn(int column) const
		{
			return {Raw[column], Raw[4 + column], Raw[8 + column], Raw[12 + column]};
		}
	...
	public:
		...
		static void Transformation(const Float3& scalingCenter,
			const Quaternion& scalingRotation,
			const Float3& scaling,
			const Float3& rotationCenter,
		...
		// @param translation The translation factor of the transformation.
		// @param result When the method completes, contains the created transformation matrix.
		static void Transformation2D(const Float2& scalingCenter,
			float scalingRotation,
			const Float2& scaling,
			const Float2& rotationCenter,
			float rotation,
	...
	public:
		static Float3 TransformVector(const Matrix& m, const Float3& v);
...
template<>
struct TIsPODType<SE::Matrix>
{
	enum { Value = true };
};

DEFINE_DEFAULT_FORMATTING(SE::Matrix, "[M11:{0} M12:{1} M13:{2} M14:{3}] [M21:{4} M22:{5} M23:{6} M24:{7}] [M31:{8} M32:{9} M33:{10} M34:{11}] [M41:{12} M42:{13} M43:{14} M44:{15}]",
