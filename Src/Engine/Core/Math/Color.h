...
namespace SE
{
    ...
    public:
        ...
        {
            r = static_cast<uint8>(c.Raw[0] * 255);
            g = static_cast<uint8>(c.Raw[1] * 255);
            b = static_cast<uint8>(c.Raw[2] * 255);
            a = static_cast<uint8>(c.Raw[3] * 255);
        }
        ...
        // Get a version with the a different alpha value
        inline Color32 GetAlphaVersion(float alpha) const
        {
            float const floatAlpha = Math::Clamp(alpha * 255, 0.0f, 255.0f);
            return GetAlphaVersion(static_cast<uint8>(floatAlpha));
        }

        // Scale the color values with a multiplier
        inline void ScaleColor(float multiplier)
        {
            r = static_cast<uint8>(Math::Clamp(Math::RoundToInt(static_cast<float>(r) * multiplier), 0, 255));
            g = static_cast<uint8>(Math::Clamp(Math::RoundToInt(static_cast<float>(g) * multiplier), 0, 255));
            b = static_cast<uint8>(Math::Clamp(Math::RoundToInt(static_cast<float>(b) * multiplier), 0, 255));
        }

        // Get a version with color values scaled
        inline Color32 GetScaledColor(float multiplier) const
        {
            Color32 c = *this;
        ...
        // Equality
        inline bool operator==(Color32 const &rhs) const { return value == rhs.value; }
        inline bool operator!=(Color32 const &rhs) const { return value != rhs.value; }

        //-------------------------------------------------------------------------

        // Returns a uint32 color with this byte format: 0xAAGGBBRR
        inline uint32 ToUInt32() const { return value; }
        ...
        inline Float4 ToFloat4() const { return Float4(static_cast<float>(r) / 255, static_cast<float>(g) / 255, static_cast<float>(b) / 255, static_cast<float>(a) / 255); }

        // Returns a float4 color where x=R, y=G, z=B, w=A
        inline operator Float4() const { return ToFloat4(); }
    };

	struct SE_API_CORE Color
	{
	public:
		union
		{
	...
	uint32 SE_API_CORE GetHash(const Color32 &color);

	uint32 SE_API_CORE GetHash(const Color &color);
}

//-------------------------------------------------------------------------

template <>
struct SE_API_CORE std::hash<SE::Color32>
{
	uint32 operator()(SE::Color32 const &color) const noexcept
	{
		return color.value;
	}
};
