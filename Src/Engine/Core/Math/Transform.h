...
namespace SE
{
	...
	public:
		...
		Transform(const Float3& position, const Quaternion& rotation, const Float3& scale)
			: Translation(position)
			, Orientation(rotation)
			, Scale(scale)
		{
		...
		Transform(const Float3& position, const Matrix3x3& rotationScale);
	...
	public:
		...
		Transform Add(const Float3& translation) const;
		...
		Float3 LocalToWorld(const Float3& point) const
		{
			Float3 result;
		...
		void LocalToWorldVector(const Float3& vector, Float3& result) const;
		...
		Float3 LocalToWorldVector(const Float3& vector) const
		{
			Float3 result;
		...
		void WorldToLocal(const Float3& point, Float3& result) const;
		...
		Float3 WorldToLocal(const Float3& point) const
		{
			Float3 result;
		...
		void WorldToLocalVector(const Float3& vector, Float3& result) const;
		...
		Float3 WorldToLocalVector(const Float3& vector) const
		{
			Float3 result;
	...
	public:
		...
		FORCE_INLINE Transform operator+(const Float3& other) const
		{
			return Add(other);
		}
		...
		{
			return Float3::NearEqual(a.Translation, b.Translation) && Quaternion::NearEqual(a.Orientation, b.Orientation) && Float3::NearEqual(a.Scale, b.Scale);
		}

		static bool NearEqual(const Transform& a, const Transform& b, float epsilon)
		{
			return Float3::NearEqual(a.Translation, b.Translation, epsilon) && Quaternion::NearEqual(a.Orientation, b.Orientation, epsilon) && Float3::NearEqual(a.Scale, b.Scale, epsilon);
		}
...
namespace Math
{
	FORCE_INLINE static bool NearEqual(const SE::Transform& a, const SE::Transform& b)
	{
		return SE::Transform::NearEqual(a, b);
	}
}

template<>
struct TIsPODType<SE::Transform>
{
	enum { Value = true };
};

DEFINE_DEFAULT_FORMATTING(SE::Transform, "Translation:{0} Orientation:{1} Scale:{2}", v.Translation, v.Orientation, v.Scale);