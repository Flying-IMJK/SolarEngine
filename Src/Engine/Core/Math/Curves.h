...
namespace SE::Math
{
    ...
    public:
        ...
        {
            return Math::IsNearEqual(Time, other.Time) && Math::IsNearEqual(Value, other.Value);
        }
    } PACK_END();
    ...
    public:
        ...
        {
            Interpolate(a.Value, b.Value, alpha, result);
        }
        ...
        FORCE_INLINE static void InterpolateKey(const LinearCurveKeyframe& a, const LinearCurveKeyframe& b, float alpha, float length, LinearCurveKeyframe& result)
        {
            ...
            Interpolate(a.Value, b.Value, alpha, result.Value);
        }
    ...
    public:
        ...
        HermiteCurveKeyframe(float time, const T& value)
        {
            Time = time;
            Value = value;
            TangentIn = TangentOut = GetZero<T>();
        }
    ...
    public:
        ...
        static void Interpolate(const HermiteCurveKeyframe& a, const HermiteCurveKeyframe& b, float alpha, float length, T& result)
        {
            ...
            CubicHermite( a.Value, b.Value, leftTangent, rightTangent, alpha, result);
        }
        ...
        static void InterpolateFirstDerivative(const HermiteCurveKeyframe& a, const HermiteCurveKeyframe& b, float alpha, float length, T& result)
        {
            T leftTangent = a.Value + a.TangentOut * length;
            T rightTangent = b.Value + b.TangentIn * length;
            CubicHermiteFirstDerivative( a.Value, b.Value, leftTangent, rightTangent, alpha, result);
        }
        ...
        static void InterpolateKey(const HermiteCurveKeyframe& a, const HermiteCurveKeyframe& b, float alpha, float length, HermiteCurveKeyframe& result)
        {
            ...
            T leftTangent = a.Value + a.TangentOut * length;
            T rightTangent = b.Value + b.TangentIn * length;
            CubicHermite(a.Value, b.Value, leftTangent, rightTangent, alpha, result.Value);
            CubicHermiteFirstDerivative(a.Value, b.Value, leftTangent, rightTangent, alpha, result.TangentIn);
        ...
        {
            return Math::IsNearEqual(Time, other.Time) && Math::IsNearEqual(Value, other.Value) && Math::IsNearEqual(TangentIn, other.TangentIn) && Math::IsNearEqual(TangentOut, other.TangentOut);
        }
    } PACK_END();
    ...
    public:
        ...
        static void Interpolate(const BezierCurveKeyframe& a, const BezierCurveKeyframe& b, float alpha, float length, T& result)
        {
            ...
            GetTangent(a.Value, a.TangentOut, length, leftTangent);
            GetTangent(b.Value, b.TangentIn, length, rightTangent);
            Bezier(a.Value, leftTangent, rightTangent, b.Value, alpha, result);
        }
        ...
        static void InterpolateFirstDerivative(const BezierCurveKeyframe& a, const BezierCurveKeyframe& b, float alpha, float length, T& result)
        {
            ...
            GetTangent(a.Value, a.TangentOut, length, leftTangent);
            GetTangent(b.Value, b.TangentIn, length, rightTangent);
            BezierFirstDerivative(a.Value, leftTangent, rightTangent, b.Value, alpha, result);
        }
        ...
        static void InterpolateKey(const BezierCurveKeyframe& a, const BezierCurveKeyframe& b, float alpha, float length, BezierCurveKeyframe& result)
        {
            ...
            GetTangent(a.Value, a.TangentOut, length, leftTangent);
            GetTangent(b.Value, b.TangentIn, length, rightTangent);
            Bezier(a.Value, leftTangent, rightTangent, b.Value, alpha, result.Value);
    ...
    public:
        ...
        void Evaluate(const KeyFrameData& data, T& result, float time, bool loop = true) const
        {
            ...
            float t = Math::IsNearEqual(length, 0.0f) ? 0.0f : (time - leftKey.Time) / length;
    ...
    protected:
        ...
        static void WrapTime(float& time, float start, float end, bool loop)
        {
            ...
            if (Math::IsNearEqual(length, 0.0f))
            {
                time = 0.0f;
                return;
            }
    ...
    public:
        ...
        using KeyFrameCollection = List<KeyFrame>;
    ...
    public:
        ...
        void Trim(float start, float end)
        {
            ...
            if (_keyframes.IsEmpty() || !Math::IsNearEqual(_keyframes.First().Time, start))
            {
            ...
            if (_keyframes.IsEmpty() || !Math::IsNearEqual(_keyframes.Last().Time, end))
            {
