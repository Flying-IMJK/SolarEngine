...
namespace SE::Threading
{
	...
	SE_API_CORE bool Init();
	SE_API_CORE void Shutdown();

	SE_API_CORE bool IsMainThread();

	SE_API_CORE void SetMainThreadID(int64 threadID);

	SE_API_CORE int64 GetMainThreadID();

	SE_API_CORE int64 GetCurrentThreadID();

	SE_API_CORE void Sleep(int64 milliseconds);

	SE_API_CORE Thread* StartThread(const Function<int32()>& callback, const String& threadName, ThreadPriority priority = ThreadPriority::Normal);

	/// <summary>
	/// General purpose task executed on Main Thread in the beginning of the next frame.
	/// </summary>
	/// <seealso cref="Task" />
	class SE_API_CORE MainThreadTask : public Task
	{
		friend class Engine;
	private:
	...
	/// <seealso cref="Task" />
	class SE_API_CORE MainThreadActionTask : public MainThreadTask
	{
	protected:
