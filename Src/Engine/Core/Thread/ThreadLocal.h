...
namespace SE::Threading
{
	...
	public:
		...
		void GetValues(List<T, AllocationType>& result) const
		{
			for (int32 i = 0; i < MaxThreads; i++)
			{
				if (Platform::AtomicRead((int64 volatile*)&_staticBuckets[i].ThreadID) != 0)
	...
	protected:
		Bucket& GetBucket()
		{
			...
			while (spaceLeft)
			{
				...
				if (value == 0 && Platform::AtomicCompareExchange(&_staticBuckets[index].ThreadID, key, 0) == 0)
					return _staticBuckets[index];
		...
		DYNAMIC:
			...
			if (!dynamicBuckets)
			{
				...
				if (Platform::AtomicCompareExchange((intptr volatile*)&_dynamicBuckets, (intptr)dynamicBuckets, 0) != 0)
				{
			...
			while (spaceLeft)
			{
				...
				if (value == 0 && Platform::AtomicCompareExchange(&dynamicBuckets[index].ThreadID, key, 0) == 0)
					return dynamicBuckets[index];
