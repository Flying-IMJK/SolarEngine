...
namespace SE::Log
{
	...
	public:
		...
		ArgumentException(const String& additionalInfo)
			: Exception(SE_TEXT("One or more of provided arguments are not valid."), additionalInfo)
		{
		}
		...
		ArgumentException(const String& argumentName, const String& additionalInfo)
			: Exception(String::Format(SE_TEXT("Provided argument {0} is not valid."), argumentName), additionalInfo)
		{
		}
	};
}
