...
{
    template<>
    struct formatter<SE::DateTime, SE::Char>
    {
        ...
        auto format(const SE::DateTime& v, FormatContext& ctx) -> decltype(ctx.out())
        {
			int32 year, month, day;
            v.GetDate(year, month, day);
            return format_to(ctx.out(), basic_string_view<SE::Char>(SE_TEXT("{0}-{1:0>2}-{2:0>2} {3:0>2}:{4:0>2}:{5:0>2}")),
                year,
				month,
				day,
				v.GetHour(),
