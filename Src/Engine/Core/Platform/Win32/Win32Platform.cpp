...
namespace SE
{
	...
	bool Win32Platform::Init()
	{
		...
		// Setup unique device ID
		{
			...
			HashCombine(hash, GetHash(Platform::GetUserName()));
			DeviceId.A = hash;
	    ...
	    while (pAdapterInfo)
	    {
			...
			while (pAdapterInfo)
			{
				hash = pAdapterInfo->PhysicalAddress[0];
				for (UINT i = 1; i < pAdapterInfo->PhysicalAddressLength; i++)
					HashCombine(hash, pAdapterInfo->PhysicalAddress[i]);
				DeviceId.B = hash;
		...
		if (WSAStartup(MAKEWORD(2, 0), &WsaData) != 0)
			LOG_ERROR("Platform", "Unable to initializes native network! Error : {0}", GetLastErrorMessage().Get());
	...
	void Win32Platform::CreateGuid(UID& result)
	{
	...
	UID Win32Platform::GetUniqueDeviceId()
	{
		return DeviceId;
	}
