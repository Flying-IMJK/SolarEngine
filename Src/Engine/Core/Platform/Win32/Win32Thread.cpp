...
#pragma warning(pop)
	...
	Win32Thread::Win32Thread(Threading::IRunnable* runnable, const String& name, ThreadPriority priority)
	...
	{
		ENGINE_ASSERT(_thread == nullptr);
	}

	Win32Thread* Win32Thread::Create(Threading::IRunnable* runnable, const String& name, ThreadPriority priority, uint32 stackSize)
	{
	...
	bool Win32Thread::Start(uint32 stackSize)
	{
		...
		// Initialize priority
		SetPriorityInternal(m_Priority);

#if WINDOWS_ENABLE_THREAD_NAMING
		SetThreadName(id, m_Name.ToStringAnsi().Get());
#endif
