...
namespace SE::Serialization
{
    ...
    SE_API_CORE bool ReadTypeDescriptorFromJSON(Serialization::JsonValue const& typeObjectValue, TypeDescriptor& outDesc );
    SE_API_CORE void WriteTypeDescriptorToJSON(Serialization::JsonWriter& writer, TypeDescriptor const& type );

    // Type Instances
    //-------------------------------------------------------------------------

    // Read the data for a native type from JSON - expect a fully created type to be supplied and will override the values
    SE_API_CORE bool ReadNativeType(Serialization::JsonValue const& typeObjectValue, IReflectedType* pTypeInstance );

    // Read the data for a native type from JSON - expect a fully created type to be supplied and will override the values
    SE_API_CORE bool ReadNativeTypeFromString(String const& jsonString, IReflectedType* pTypeInstance );

    // Serialize a supplied native type to JSON - creates a new JSON object for this type
    SE_API_CORE void WriteNativeType(IReflectedType const* pTypeInstance, Serialization::JsonWriter& writer );
    ...
    SE_API_CORE void WriteNativeTypeContents(IReflectedType const* pTypeInstance, Serialization::JsonWriter& writer );

    // Write the property data for a supplied native type to JSON
    SE_API_CORE void WriteNativeTypeToString(IReflectedType const* pTypeInstance, String& outString );

    // Create a new instance of a type from a supplied JSON version
    SE_API_CORE IReflectedType* TryCreateAndReadNativeType(Serialization::JsonValue const& typeObjectValue );
    ...
    class SE_API_CORE TypeArchiveReader : public JsonArchiveReader
    {
    public:
    ...
    class SE_API_CORE TypeArchiveWriter final : public JsonArchiveWriter
    {
    public:
