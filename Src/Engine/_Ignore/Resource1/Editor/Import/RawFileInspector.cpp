...
{
    /*
    static bool InspectFBX( InspectorContext const& ctx, FileSystem::Path const& sourceFilePath, Vector<ImportableItem*>& outFileInfo )
    {
        ResPath const resourcePath = ResPath::FromFileSystemPath( ctx.m_rawResourceDirectoryPath, sourceFilePath );
    ...
    static bool InspectGLTF( InspectorContext const& ctx, FileSystem::Path const& sourceFilePath, Vector<ImportableItem*>& outFileInfo )
    {
        ResPath const resourcePath = ResPath::FromFileSystemPath( ctx.m_rawResourceDirectoryPath, sourceFilePath );
    ...
    static bool InspectImage( InspectorContext const& ctx, FileSystem::Path const& sourceFilePath, Vector<ImportableItem*>& outFileInfo )
    {
        ImportableImage* pImg = EE::New<ImportableImage>();
        pImg->m_sourceFile = ResPath::FromFileSystemPath( ctx.m_rawResourceDirectoryPath, sourceFilePath );
    ...
    */
    ...
    InspectionResult InspectFile( InspectorContext const& ctx, String const& sourceFilePath, List<ImportableItem*>& outFileInfo )
    {
        ENGINE_ASSERT( ctx.IsValid() && !sourceFilePath.IsEmpty() );
        ENGINE_ASSERT( outFileInfo.IsEmpty() );

        String extension = FileSystem::GetExtension(sourceFilePath).ToLower();
        //-------------------------------------------------------------------------

        int32 const numValidExtensions = sizeof( g_importableExtensions ) / sizeof( g_importableExtensions[0] );
        int32 matchingExtensionIdx = -1;

        for ( int32 i = 0; i < numValidExtensions; i++ )
        {
    ...
    bool IsImportableFileType(String const& extension )
    {
