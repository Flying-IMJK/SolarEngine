...
namespace SE
{
    ...
    class SE_API_RUNTIME Entity : public IReflectedType
    {
        SE_CLASS( Entity, IReflectedType)
        ...
        using SystemUpdateList = List<EntitySystem*>;
    ...
    public:
        ...
        void GetReferencedResources( List<ResID>& outReferencedResources ) const;
        ...
        inline OBB const& GetRootSpatialComponentWorldBounds() const { ENGINE_ASSERT( IsSpatialEntity() ); return m_pRootSpatialComponent->GetWorldBounds(); }
        ...
        inline Transform const& GetLocalTransform() const { ENGINE_ASSERT( IsSpatialEntity() ); return m_pRootSpatialComponent->GetLocalTransform(); }

        // Get the world transform for this entity i.e. the world transform of the root spatial component
        inline Transform const& GetWorldTransform() const { ENGINE_ASSERT( IsSpatialEntity() ); return m_pRootSpatialComponent->GetWorldTransform(); }
        
        // Set the world transform for this entity i.e. the transform of the root spatial component
        inline void SetWorldTransform( Transform const& worldTransform ) const { ENGINE_ASSERT( IsSpatialEntity() ); return m_pRootSpatialComponent->SetWorldTransform( worldTransform ); }
        ...
        inline EntityID const& GetSpatialParentID() const { ENGINE_ASSERT( HasSpatialParent() ); return m_pParentSpatialEntity->GetID(); }
        ...
        inline StringID const& GetAttachmentSocketID() const { ENGINE_ASSERT( HasSpatialParent() ); return m_parentAttachmentSocketID; }
        
        // Get the actual world space socket transform that we are attached to
        inline Transform GetAttachmentSocketTransform( StringID socketID ) const { ENGINE_ASSERT( IsSpatialEntity() ); return m_pRootSpatialComponent->GetAttachmentSocketTransform( socketID ); }

        // Do we have any entities attached to us
        inline bool HasAttachedEntities() const { return !m_attachedEntities.IsEmpty(); }

        // Get all attached entities
		List<Entity*> const& GetAttachedEntities() const { return m_attachedEntities; }
        ...
        inline bool HasStateChangeActionsPending() const { return !m_deferredActions.IsEmpty(); }
        ...
        inline uint32_t GetNumComponents() const { return (uint32_t) m_components.Count(); }

        inline List<EntityComponent*> const& GetComponents() const { return m_components; }
        ...
        inline EntityComponent const* FindComponent( ComponentID const& componentID ) const
        {
			...
			int foundIndex = ListExtensions::IndexOf(m_components, predicate);
            return foundIndex != INVALID_INDEX ? m_components[foundIndex] : nullptr;
        }
        ...
        void CreateComponent( TypeInfo const* pComponentTypeInfo, ComponentID const& parentSpatialComponentID = ComponentID() );
        ...
        inline uint32_t GetNumSystems() const { return (uint32_t) m_systems.Count(); }

        // Get all systems
        inline List<EntitySystem*> const& GetSystems() const { return m_systems; }
        ...
        T* GetSystem()
        {
            ...
            {
                if ( pSystem->GetTypeInfo()->id == T::GetStaticTypeID() )
                {
        ...
        void CreateSystem( TypeInfo const* pSystemTypeInfo );

        // Request creation of a new system
        template<typename T> 
        inline void CreateSystem()
        {
            static_assert( std::is_base_of<EntitySystem, T>::value, "Invalid system type detected" );
            ENGINE_ASSERT( !VectorContains( m_systems, T::s_pTypeInfo->id, [] ( EntitySystem* pSystem, TypeID systemTypeID ) { return pSystem->GetTypeInfo()->id == systemTypeID; } ) );
            CreateSystem( T::s_pTypeInfo );
        }

        // Destroy an existing system
        void DestroySystem( TypeID systemTypeID );

        // Destroy an existing system
        void DestroySystem( TypeInfo const* pSystemTypeInfo );
        ...
        {
			Function<bool(EntityComponent* const &)> predicate = [&componentID] (EntityComponent* const &pComponent) {
			  return pComponent->GetNameID() == componentID; };
            auto foundIndex = ListExtensions::IndexOf(m_components, predicate);
    ...
    private:
        ...
        // This function will search through the spatial hierarchy of this entity and return the first component it finds that contains a socket with the specified socket ID
        inline SpatialEntityComponent* FindSocketAttachmentComponent( StringID socketID ) const { ENGINE_ASSERT( IsSpatialEntity() ); return FindSocketAttachmentComponent( m_pRootSpatialComponent, socketID ); }
        ...
        void CreateSystemImmediate( TypeInfo const* pSystemTypeInfo );
        void DestroySystemImmediate( TypeInfo const* pSystemTypeInfo );
    ...
    protected:
		...
		List<EntitySystem*>                                 m_systems;
		List<EntityComponent*>                              m_components;
//        SystemUpdateList                                    m_systemUpdateLists[(int8_t) UpdateStage::NumStages];
		...
		List<Entity*>                                       m_attachedEntities;                                                     // The list of entities that are attached to this entity
        Entity*                                             m_pParentSpatialEntity = nullptr;                                       // The parent entity we are attached to
        SE_PROPERTY() StringID                           m_parentAttachmentSocketID;                                             // The socket that we are attached to on the parent
        bool                                                m_isSpatialAttachmentCreated = false;                                   // Has the actual component-to-component attachment been created

		List<EntityInternalStateAction>                     m_deferredActions;                                                      // The set of internal entity state changes that need to be executed
        CriticalSection                          			m_internalStateMutex;                                                   // A mutex that needs to be lock due to internal state changes
    };
 }