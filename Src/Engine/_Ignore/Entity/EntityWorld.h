...
#include "Runtime/Settings/SettingsRegistry.h"
#include "Runtime/Render/Drawing/DebugDrawingSystem.h"
...
namespace SE
{
    ...
    class SE_API_RUNTIME EntityWorld
    {
    ...
    public:
        ...
        void Initialize( Systems const& systemsRegistry, List<TypeInfo const*> worldSystemTypeInfos );
        void Shutdown();
        ...
        // Get world settings
        ISettings* GetSettings( TypeInfo const* pTypeInfo ) { return m_pSettingsRegistry->GetSettings( m_worldID.m_value, pTypeInfo ); }

        // Get world settings
        ISettings* GetMutableSettings( TypeInfo const* pTypeInfo ) const { return const_cast<EntityWorld*>( this )->GetSettings( pTypeInfo ); }

        // Get world settings
        ISettings const* GetSettings( TypeInfo const* pTypeInfo ) const { return const_cast<EntityWorld*>( this )->GetSettings( pTypeInfo ); }
        ...
        inline void RequestTimeStep() { ENGINE_ASSERT( IsPaused() ); m_timeStepRequested = true; }
        ...
        inline void SetTimeStepLength( Seconds newTimeStepLength ) { ENGINE_ASSERT( newTimeStepLength > 0 ); m_timeStepLength = newTimeStepLength; }

        //-------------------------------------------------------------------------
        // Viewport
        //-------------------------------------------------------------------------

/*        inline Render::RenderViewport* GetViewport() { return &m_viewport; }
        inline Render::RenderViewport const* GetViewport() const { return &m_viewport; }*/
        ...
        EntityModel::EntityMap* GetFirstNonPersistentMap() { return ( m_maps.Count() > 1 ) ? m_maps[1] : nullptr; }

        // Get the first non-persistent map
        EntityModel::EntityMap const* GetFirstNonPersistentMap() const { return ( m_maps.Count() > 1 ) ? m_maps[1] : nullptr; }

        // Create a transient map (one that is managed programatically)
        EntityModel::EntityMap* CreateTransientMap();

        // Get a specific map
        EntityModel::EntityMap const* GetMap( ResID const& mapResourceID ) const;

        // Get a specific map
        EntityModel::EntityMap* GetMap( ResID const& mapResourceID ) { return const_cast<EntityModel::EntityMap*>( const_cast<EntityWorld const*>( this )->GetMap( mapResourceID ) ); }
        ...
        EntityMapID LoadMap( ResID const& mapResourceID );
        void UnloadMap( ResID const& mapResourceID );
        ...
        inline List<EntityComponent const*> const& GetAllRegisteredComponentsOfType( TypeID typeID ) { return m_componentTypeLookup[typeID]; }

        // Get all the registered components of the specified type
        template<typename T>
        inline List<T const*> GetAllRegisteredComponentsOfType() 
        {
            List<T const*> results;
        ...
        inline void SetDebugName( char const* pName ) { ENGINE_ASSERT( pName != nullptr ); m_debugName = pName; }
    ...
    private:
        ...
        List<EntityWorldSystem*>                                                m_worldSystems;
        EntityWorldType                                                         m_worldType = EntityWorldType::Game;
        bool                                                                    m_initialized = false;
        bool                                                                    m_isSuspended = false;
//        Render::RenderViewport                                                  m_viewport = Render::RenderViewport( Int2::Zero, Int2( 640, 480 ), Render::ViewVolume( Float2( 640, 480 ), FloatRange( 0.1f, 100.0f ) ) );

        // Maps
        List<EntityModel::EntityMap*>                                        	m_maps;

        // Entities
        List<Entity*>                                                        	m_entityUpdateList;
//        List<EntityWorldSystem*>                                             	m_systemUpdateLists[(int8_t) UpdateStage::NumStages];
