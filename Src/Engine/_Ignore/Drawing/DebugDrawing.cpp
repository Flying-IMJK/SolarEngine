...
namespace SGE::Drawing
{
    namespace
    {
        ...
        static void CreateCircleVertices(Float4 *pVerts, VectorSIMD planeVector, VectorSIMD upAxis)
        {
            Radians const radiansPerEdge(Math::TWO_PI / g_numCircleVertices);
        ...
        {
            CreateCircleVertices(g_circleVerticesXUp, VectorSIMD::UnitY, VectorSIMD::UnitX);
            CreateCircleVertices(g_circleVerticesYUp, VectorSIMD::UnitX, VectorSIMD::UnitY);
            CreateCircleVertices(g_circleVerticesZUp, VectorSIMD::UnitX, VectorSIMD::UnitZ);
            return true;
        }
    ...
    void DrawContext::DrawPlane(Float4 const &planeEquation, Float4 const &color, DepthTest depthTestState, Seconds TTL)
    {
        ...
        auto const translation = plane.ProjectPoint(VectorSIMD::UnitW);
        auto const rotation = Quaternion::FromRotationBetweenNormalizedVectors(VectorSIMD::UnitZ, plane.GetNormal());
    ...
    void DrawContext::DrawBox(Transform const &transform, Float3 const &halfsize, Float4 const &color, DepthTest depthTestState, Seconds TTL)
    {
        ...
        VectorSIMD verts[8] =
            {
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[0]) * halfsize),
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[1]) * halfsize),
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[2]) * halfsize),
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[3]) * halfsize),
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[4]) * halfsize),
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[5]) * halfsize),
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[6]) * halfsize),
                transform.TransformPoint(VectorSIMD(g_unitCubeVertices[7]) * halfsize)};
    ...
    void DrawContext::DrawCircle(Transform const &transform, Axis upAxis, float radius, Float4 const &color, float lineThickness, DepthTest depthTestState, Seconds TTL)
    {
		...
		auto verts = (VectorSIMD*)PlatformAllocator::Allocate(sizeof(VectorSIMD) * g_numCircleVertices);
        for (auto i = 0; i < g_numCircleVertices; i++)
        {
    ...
    void DrawContext::DrawDisc(Float3 const &worldPoint, float radius, Float4 const &color, DepthTest depthTestState, Seconds TTL)
    {
        ...
        // Create and transform vertices
        VectorSIMD const radiusVector(radius);
        VectorSIMD const worldPointVector(worldPoint);

        auto verts = (VectorSIMD*)PlatformAllocator::Allocate(sizeof(VectorSIMD) * numVerts);
        for (auto i = 0; i < numVerts; i++)
        {
            verts[i] = VectorSIMD::MultiplyAdd(g_circleVerticesZUp[i], radiusVector, worldPointVector);
        }
    ...
    {
        VectorSIMD const axisX = worldTransform.GetAxisX();
        VectorSIMD const axisY = worldTransform.GetAxisY();
        VectorSIMD const axisZ = worldTransform.GetAxisZ();

        VectorSIMD const origin = worldTransform.GetTranslation();
        VectorSIMD const halfHeightOffset = (axisZ * halfHeight);

        VectorSIMD const cylinderCenterTop = origin + halfHeightOffset;
        VectorSIMD const cylinderCenterBottom = origin - halfHeightOffset;

        // 8 lines
        //-------------------------------------------------------------------------

        VectorSIMD xOffset = (axisX * radius);
        VectorSIMD lt0 = cylinderCenterTop + xOffset;
        VectorSIMD lt1 = cylinderCenterTop - xOffset;
        VectorSIMD lb0 = cylinderCenterBottom + xOffset;
        VectorSIMD lb1 = cylinderCenterBottom - xOffset;

        DrawLine(lt0, lb0, color, thickness, depthTestState, TTL);
        DrawLine(lt1, lb1, color, thickness, depthTestState, TTL);

        VectorSIMD yOffset = (axisY * radius);
        VectorSIMD lt2 = cylinderCenterTop + yOffset;
        VectorSIMD lt3 = cylinderCenterTop - yOffset;
        VectorSIMD lb2 = cylinderCenterBottom + yOffset;
        VectorSIMD lb3 = cylinderCenterBottom - yOffset;

        DrawLine(lt2, lb2, color, thickness, depthTestState, TTL);
        DrawLine(lt3, lb3, color, thickness, depthTestState, TTL);

        VectorSIMD xzOffset0 = (axisX + axisY).GetNormalized3() * radius;
        VectorSIMD lt4 = cylinderCenterTop + xzOffset0;
        VectorSIMD lt5 = cylinderCenterTop - xzOffset0;
        VectorSIMD lb4 = cylinderCenterBottom + xzOffset0;
        VectorSIMD lb5 = cylinderCenterBottom - xzOffset0;

        DrawLine(lt4, lb4, color, thickness, depthTestState, TTL);
        DrawLine(lt5, lb5, color, thickness, depthTestState, TTL);

        VectorSIMD xzOffset1 = (axisX - axisY).GetNormalized3() * radius;
        VectorSIMD lt6 = cylinderCenterTop + xzOffset1;
        VectorSIMD lt7 = cylinderCenterTop - xzOffset1;
        VectorSIMD lb6 = cylinderCenterBottom + xzOffset1;
        VectorSIMD lb7 = cylinderCenterBottom - xzOffset1;
        ...
        if (isCapsule)
        {
            ...
            auto DrawSemiCircle = [&](VectorSIMD const &startPoint, VectorSIMD const &capCenterPoint, VectorSIMD const &shapeOrigin)
            {
                VectorSIMD planeVector = startPoint - capCenterPoint;
                VectorSIMD const rotationAxis = (startPoint - capCenterPoint).Cross3(startPoint - shapeOrigin).GetNormalized3();
                Quaternion const rotation(rotationAxis, radiansPerEdge);

                VectorSIMD prevPlaneVector = planeVector;
    ...
    void DrawContext::DrawArrow(Float3 const &startPoint, Float3 const &endPoint, Float4 const &color, float thickness, DepthTest depthTestState, Seconds TTL)
    {
        ...
        VectorSIMD const vStart(startPoint);
        VectorSIMD const vEnd(endPoint);

        VectorSIMD arrowDirection;
        float arrowLength;
        ...
        VectorSIMD const vArrowHeadStartPoint = vStart + (arrowDirection * arrowHeadLength);
    ...
    {
        VectorSIMD const capOffset = (transform.GetForwardVector() * length);
