...
namespace SE
{
    ...
    void MeshData::InitFromModelVertices(ModelVertex* vertices, uint32 verticesCount)
    {
    ...
    void MeshData::InitFromModelVertices(VB0ElementType* vb0, VB1ElementType* vb1, uint32 verticesCount)
    {
        Positions.Resize(verticesCount, false);
        UVs.Resize(verticesCount, false);
        Normals.Resize(verticesCount, false);
    ...
    void MeshData::InitFromModelVertices(VB0ElementType* vb0, VB1ElementType* vb1, VB2ElementType* vb2, uint32 verticesCount)
    {
        Positions.Resize(verticesCount, false);
        UVs.Resize(verticesCount, false);
        Normals.Resize(verticesCount, false);
    ...
    bool MeshData::Pack2Model(WriteStream* stream) const
    {
        ...
        if (stream == nullptr)
        {
            LOG_ERROR("Resource", "Invalid input.");
            return false;
        }
        ...
        {
            LOG_ERROR("Resource", "Empty mesh! Triangles: {0}, Verticies: {1}.", trianglesCount, verticiecCount);
            return false;
        }
    ...
    bool MeshData::Pack2SkinnedModel(WriteStream* stream) const
    {
        ...
        {
            LOG_ERROR("Resource", "Invalid size of {0} stream.", SE_TEXT("BlendIndices"));
            return true;
        }
        if (BlendWeights.Count() != verticiecCount)
        {
            LOG_ERROR("Resource", "Invalid size of {0} stream.", SE_TEXT("BlendWeights"));
            return true;
        }
    ...
    void MeshData::NormalizeBlendWeights()
    {
        ...
        for (int32 i = 0; i < Positions.Count(); i++)
        {
            ...
            const float invSum = sum > Math::ZeroTolerance ? 1.0f / sum : 0.0f;
            weights *= invSum;
        }
    }
...
#undef MERGE
    ...
    {
        return Diffuse.Color != Colors::White ||
                Diffuse.TextureIndex != -1 ||
                Emissive.Color != Colors::Transparent ||
                Emissive.TextureIndex != -1 ||
                !Math::IsOne(Opacity.Value) ||
                Opacity.TextureIndex != -1 ||
                !Math::IsNearEqual(Roughness.Value, 0.5f) ||
    ...
    bool ModelData::Pack2ModelHeader(WriteStream* stream) const
    {
        ...
        {
            Log::ArgumentOutOfRangeException(SE_TEXT("MaterialSlots"), SE_TEXT("Material slots collection cannot be empty."));
            return true;
        }
        ...
        for (int32 lodIndex = 0; lodIndex < lodCount; lodIndex++)
        {
            ...
            if (meshes == 0)
            {
                LOG_WARNING("Resource", "Empty LOD.");
                return true;
            }
            if (meshes > MODEL_MAX_MESHES)
            {
                LOG_WARNING("Resource", "Too many meshes per LOD.");
                return true;
            }
    ...
    bool ModelData::Pack2AnimationHeader(WriteStream* stream, int32 animIndex) const
    {
        ...
        {
            Log::InvalidOperationException(SE_TEXT("Invalid animation duration."));
            return true;
        }
        if (anim.Channels.IsEmpty())
        {
            Log::ArgumentOutOfRangeException(SE_TEXT("Channels"), SE_TEXT("Animation channels collection cannot be empty."));
            return true;
        }
