...
#include "Runtime/Render/PostProcessSettings.h"
...
{
    return SE_TEXT("PostProcessingPass");
}
...
bool PostProcessingPass::Init()
	{
	    ...
	    _psThreshold = device->CreatePipelineState();
	    _psScale = device->CreatePipelineState();
	    _psBlurH = device->CreatePipelineState();
	    _psBlurV = device->CreatePipelineState();
	    _psGenGhosts = device->CreatePipelineState();
	    _psComposite.CreatePipelineStates();

	    // Load shader
	    _shader = AssetContent::LoadAsyncInternal<Shader>(SE_TEXT("Shaders/PostProcessing"));
	    if (_shader == nullptr)
	    {
	        return false;
	    }
...
bool PostProcessingPass::SetupResources()
{
    ...
    {
        psDesc.PS = shader->GetPS(SE_TEXT("PS_Threshold"));
        if (!_psThreshold->Init(psDesc))
            return false;
    }
    if (!_psScale->IsValid())
    {
        psDesc.PS = shader->GetPS(SE_TEXT("PS_Scale"));
        if (!_psScale->Init(psDesc))
            return false;
    }
    if (!_psBlurH->IsValid())
    {
        psDesc.PS = shader->GetPS(SE_TEXT("PS_GaussainBlurH"));
        if (!_psBlurH->Init(psDesc))
            return false;
    }
    if (!_psBlurV->IsValid())
    {
        psDesc.PS = shader->GetPS(SE_TEXT("PS_GaussainBlurV"));
        if (!_psBlurV->Init(psDesc))
            return false;
    }
    if (!_psGenGhosts->IsValid())
    {
        psDesc.PS = shader->GetPS(SE_TEXT("PS_Ghosts"));
        if (!_psGenGhosts->Init(psDesc))
            return false;
    }
    if (!_psComposite.IsValid())
    {
        if (!_psComposite.Create(psDesc, shader, SE_TEXT("PS_Composite")))
            return false;
    }
...
GPUTexture* PostProcessingPass::GetCustomOrDefault(Texture* customTexture, AssetRef<Texture>& defaultTexture, const Char* defaultName)
{
    ...
    {
        // Load default
        defaultTexture = AssetContent::LoadAsyncInternal<Texture>(defaultName);
    }
...
void PostProcessingPass::GB_ComputeKernel(float sigma, float width, float height)
{
    ...
    float sigmaRoot = Math::Sqrt(twoSigmaSquare * Math::PI);
...
void PostProcessingPass::Dispose()
{
    ...
    // Cleanup
    DeleteObjectSafe(_psThreshold);
    DeleteObjectSafe(_psScale);
    DeleteObjectSafe(_psBlurH);
    DeleteObjectSafe(_psBlurV);
    DeleteObjectSafe(_psGenGhosts);
    _psComposite.Delete();
    _shader = nullptr;
    _defaultLensColor = nullptr;
    _defaultLensDirt = nullptr;
...
void PostProcessingPass::Render(RenderContext& renderContext, GPUTexture* input, GPUTexture* output, GPUTexture* colorGradingLUT)
{
    ...
    PostProcessSettings& settings = renderContext.list->PostSettings;
    bool useBloom = false;// EnumHasAnyFlags(view.Flags, ViewFlags::Bloom) && settings.Bloom.Enabled && settings.Bloom.Intensity > 0.0f;
    bool useToneMapping = /*EnumHasAnyFlags(view.Flags, ViewFlags::ToneMapping) && */settings.ToneMapping.Mode != ToneMappingMode::None;
    bool useCameraArtifacts = false;//EnumHasAnyFlags(view.Flags, ViewFlags::CameraArtifacts) && (settings.CameraArtifacts.VignetteIntensity > 0.0f || settings.CameraArtifacts.GrainAmount > 0.0f || settings.CameraArtifacts.ChromaticDistortion > 0.0f || settings.CameraArtifacts.ScreenFadeColor.A > 0.0f);
    bool useLensFlares = false;//EnumHasAnyFlags(view.Flags, ViewFlags::LensFlares) && settings.LensFlares.Intensity > 0.0f && useBloom;
    ...
    Data data;
    float time = Time::Render.UnscaledTime.GetTotalSeconds();
    ...
    else
    {
        ...
        data.ScreenFadeColor = Colors::Transparent;
    }
    ...
    data.PostExposure = Math::Exp2(8 /*settings.EyeAdaptation.PostExposure*/);
    ...
    // - 7 - ColorGradingLUT
    context->BindSR(0, input->View());
    // context->BindSR(4, GetCustomOrDefault(settings.LensFlares.LensDirt, _defaultLensDirt, SE_TEXT("Engine/Textures/DefaultLensDirt")));
