...
namespace SE
{
    struct FontManagerData
    {
        ...
        List<AssetRef<FontTextureAtlas>> Atlases;
        List<byte> GlyphImageData;
    };

    FontManagerData* fontData;


    class FontManagerSystem : public ISystem
    {
        ENGINE_SYSTEM(FontManagerSystem)
    public:
        FontManagerSystem()
            : ISystem(SE_TEXT("Font Manager"), -700)
        {
        }
    ...
    {
        return PlatformAllocator::Realloc(ptr, oldSize, newSize);
    }
    ...
    bool FontManagerSystem::OnInit()
    {
        fontData = New<FontManagerData>();

        ENGINE_ASSERT(fontData->Library == nullptr);
        ...
        const FT_Error error = FT_New_Library(&FreeTypeMemory, &fontData->Library);
        if (error)
        {
            LOG_FT_ERROR(error);
            return false;
        }
        FT_Add_Default_Modules(fontData->Library);

        // Log version info
        FT_Int major, minor, patch;
        FT_Library_Version(fontData->Library, &major, &minor, &patch);
        LOG_INFO("Resource", "FreeType initialized, version: {0}.{1}.{2}", major, minor, patch);

        return true;
    }

    void FontManagerSystem::OnDispose()
    {
        // Release font atlases
        fontData->Atlases.Resize(0);

        // Clean library
        if (fontData->Library)
        {
            const FT_Error error = FT_Done_Library(fontData->Library);
            fontData->Library = nullptr;
            if (error)
            {
                LOG_FT_ERROR(error);
            }
    ...
    {
        return index >= 0 && index < fontData->Atlases.Count() ? fontData->Atlases.Get()[index].Get() : nullptr;
    }

    bool FontManager::AddNewEntry(Font* font, Char c, FontCharacterEntry& entry)
    {
        Threading::ScopeLock lock(fontData->Locker);
        ...
        if (glyphIndex == 0 && c >= '!')
        {
            LOG_WARNING("Resource", "Font `{0}` doesn't contain character `\\u{1}`, consider choosing another font.", StringAnsiView(face->family_name), StringView(&c));
        }
#endif
        ...
        if (bitmap->pixel_mode != FT_PIXEL_MODE_GRAY)
        {
            ...
            FT_Bitmap_Convert(fontData->Library, bitmap, &tmpBitmap, 4);
            bitmap = &tmpBitmap;
        }
        ...
        fontData->GlyphImageData.Clear();
        fontData->GlyphImageData.Resize(glyphWidth * glyphHeight);

        // End for empty glyphs
        if (fontData->GlyphImageData.IsEmpty())
        {
            entry.TextureIndex = Max_uint8;
            if (bitmap == &tmpBitmap)
            {
                FT_Bitmap_Done(fontData->Library, bitmap);
                bitmap = nullptr;
            }
            return false;
        }
        ...
        {
            Platform::MemoryCopy(&fontData->GlyphImageData[row * glyphWidth], &bitmap->buffer[row * bitmap->pitch], glyphWidth);
        }
        ...
        if (bitmap->num_grays != 256)
        {
            ...
            for (byte& pixel : fontData->GlyphImageData)
            {
                pixel *= scale;
            }
        }
        ...
        for (; atlasIndex < fontData->Atlases.Count(); atlasIndex++)
        {
            ...
            slot = fontData->Atlases[atlasIndex]->AddEntry(glyphWidth, glyphHeight, fontData->GlyphImageData);
        ...
        {
            // Create new atlas
            auto atlas = AssetContent::CreateVirtualAsset<FontTextureAtlas>();
            atlas->Setup(PixelFormat::R8_UNorm, FontTextureAtlas::PaddingStyle::PadWithZero);
            fontData->Atlases.Add(atlas);
            ...
            // Add the character to the texture
            slot = atlas->AddEntry(glyphWidth, glyphHeight, fontData->GlyphImageData);
        }
        if (slot == nullptr)
        {
            LOG_ERROR("Resource", "Cannot find free space in texture atlases for character '{0}' from font {1} {2}. Size: {3}x{4}", StringView(&c),
                StringAnsiView(face->family_name), StringAnsiView(face->style_name), glyphWidth, glyphHeight);
            return true;
        }
    ...
    {
        for (const auto& atlas : fontData->Atlases)
        {
            atlas->Flush();
        }
    }
    ...
    {
        fontData->Atlases[index]->EnsureTextureCreated();
    }
