...
#include "Runtime/Module/Renderer/RendererSystem.h"
#include "Runtime/Server/GraphicsRenderer/RHI.h"
...
namespace SE
{
    ...
    void DebugDrawManager::preparePassData(Ref<RenderResourceBase> render_resource)
    {
    ...
    void DebugDrawManager::swapDataToRender()
    {
        ...
        size_t debug_draw_group_count = m_debug_draw_context.m_DebugDrawGroups.size();
        for (size_t debug_draw_group_index = 0; debug_draw_group_index < debug_draw_group_count; debug_draw_group_index++)
        {
            DebugDrawGroup* debug_draw_group = m_debug_draw_context.m_DebugDrawGroups[debug_draw_group_index];
    ...
    void DebugDrawManager::draw(uint32_t current_swapchain_image_index)
    {
        ...
        RHISwapChainDesc chainInfo = rhi.GetSwapchainInfo();

        rhi.PushEvent(cmd, "DebugDrawManager", color);
        rhi.CmdSetViewport(cmd, 0, 1, chainInfo.viewport);
        rhi.CmdSetScissor(cmd, 0, 1, chainInfo.scissor);
    ...
    void DebugDrawManager::prepareDrawBuffer()
    {
        ...
        std::vector<std::pair<Matrix4x4, Float4> > dynamicObject = { std::make_pair(Matrix4x4::IDENTITY,Float4(0,0,0,0)) };
    ...
    void DebugDrawManager::drawPointLineTriangleBox(uint32_t current_swapchain_image_index)
    {
        ...
        rhi.CmdBindVertexBuffers(cmd, 0, 1, vertex_buffers, offsets);
        ...
        renderpass_begin_info.renderArea.extent = rhi.GetSwapchainInfo().extent;
        ...
        for (size_t i = 0; i < vc_pipelines.size(); i++)
        {
            ...
            rhi.CmdBeginRenderPass(cmd, &renderpass_begin_info, RHI_SUBPASS_CONTENTS_INLINE);

            rhi.CmdBindPipeline(cmd, RHI_PIPELINE_BIND_POINT_GRAPHICS, vc_pipelines[i]->getPipeline().pipeline);

            uint32_t dynamicOffset = 0;
            rhi.CmdBindDescriptorSets(cmd,
                ...
                1,
                &dynamicOffset);
            rhi.CmdDraw(cmd, vc_end_offsets[i] - vc_start_offsets[i], 1, vc_start_offsets[i], 0);
    ...
    void DebugDrawManager::drawWireFrameObject(uint32_t current_swapchain_image_index)
    {
        ...
        for (int32_t i = 0; i < 2; i++)
        {
            ...
            if (sphere_count > 0)
            {
                ...
                rhi.CmdBindVertexBuffers(cmd, 0, 1, sphere_vertex_buffers, offsets);
                for (size_t j = 0; j < sphere_count; j++)
                {
                    ...
                    rhi.CmdBindDescriptorSets(cmd,
                        ...
                        1,
                        &dynamicOffset);
                    rhi.CmdDraw(cmd, m_buffer_allocator->getSphereVertexBufferSize(), 1, 0, 0);
            ...
            if (cylinder_count > 0)
            {
                ...
                rhi.CmdBindVertexBuffers(cmd, 0, 1, cylinder_vertex_buffers, offsets);
                for (size_t j = 0; j < cylinder_count; j++)
                {
                    rhi.CmdBindDescriptorSets(cmd,
                        ...
                        1,
                        &dynamicOffset);
                    rhi.CmdDraw(cmd, m_buffer_allocator->getCylinderVertexBufferSize(), 1, 0, 0);
                    dynamicOffset += uniform_dynamic_size;
                }
            }
            ...
            if (capsule_count > 0)
            {
                ...
                rhi.CmdBindVertexBuffers(cmd, 0, 1, capsule_vertex_buffers, offsets);
                for (size_t j = 0; j < capsule_count; j++)
                {
                    ...
                    rhi.CmdBindDescriptorSets(cmd,
                        ...
                        1,
                        &dynamicOffset);
                    rhi.CmdDraw(cmd, m_buffer_allocator->getCapsuleVertexBufferUpSize(), 1, 0, 0);
                    dynamicOffset += uniform_dynamic_size;
                    ...
                    rhi.CmdBindDescriptorSets(cmd,
                        ...
                        1,
                        &dynamicOffset);
                    rhi.CmdDraw(cmd, m_buffer_allocator->getCapsuleVertexBufferMidSize(), 1, m_buffer_allocator->getCapsuleVertexBufferUpSize(), 0);
                    dynamicOffset += uniform_dynamic_size;
