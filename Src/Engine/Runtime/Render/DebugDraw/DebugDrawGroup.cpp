...
namespace SE
{
    ...
    void DebugDrawGroup::addPoint(const Float3& position, const Float4& color, const float life_time, const bool no_depth_test)
    {
        std::lock_guard<std::mutex> guard(m_Mutex);
    ...
    void DebugDrawGroup::addLine(const Float3& point0, 
                                 const Float3& point1,
                                 const Float4& color0, 
                                 const Float4& color1, 
    ...
    void DebugDrawGroup::addTriangle(const Float3& point0,
                                     const Float3& point1,
                                     const Float3& point2,
                                     const Float4& color0,
                                     const Float4& color1,
    ...
    void DebugDrawGroup::addQuad(const Float3& point0,
                                 const Float3& point1,
                                 const Float3& point2,
                                 const Float3& point3,
                                 const Float4& color0,
    ...
    void DebugDrawGroup::addBox(const Float3& center_point,
    ...
    void DebugDrawGroup::addSphere(const Float3& center,
                                   const float    radius,
                                   const Float4& color,
                                   const float    life_time,
                                   const bool     no_depth_test)
    ...
    void DebugDrawGroup::addCylinder(const Float3& center, 
                                     const float    radius, 
                                     const float    height, 
                                     const Float4& rotate,
                                     const Float4& color, 
    ...
    void DebugDrawGroup::addCapsule(const Float3& center,
    ...
    void DebugDrawGroup::writeLineData(std::vector<DebugDrawVertex> &vertexs, bool no_depth_test)
    {
        ...
        for (DebugDrawBox box : m_boxes)
        {
            if (box.m_no_depth_test == no_depth_test)
            {
                ...
                {
                    Float3 v(f[i & 1] * box.m_half_extents.x, f[(i >> 1) & 1] * box.m_half_extents.y, f[(i >> 2) & 1] * box.m_half_extents.z);
                    Float3 uv, uuv;
                    Float3 qvec(box.m_rotate.x, box.m_rotate.y, box.m_rotate.z);
    ...
    {
        RHISwapChainDesc swapChainDesc = RHI::Instance().GetSwapchainInfo();
        ...
        for(DebugDrawText text : m_texts)
        {
            ...
            Float3 coordinate = text.m_coordinate;
            if (!text.m_is_screen_text)
            {
                Float4 tempCoord(coordinate.x, coordinate.y, coordinate.z, 1.0f);
                tempCoord = m_proj_view_matrix * tempCoord;
                coordinate = Float3(tempCoord.x / tempCoord.w, tempCoord.y / tempCoord.w, 0.0f);
            }
            ...
            for (unsigned char character : text.m_content)
            {
                ...
                else
                {
                    ...
                    vertexs[current_index].pos = Float3(cx1, cy1, 0.0f);
                    vertexs[current_index].color = text.m_color;
                    vertexs[current_index++].texcoord = Float2(x1, y1);

                    vertexs[current_index].pos = Float3(cx1, cy2, 0.0f);
                    vertexs[current_index].color = text.m_color;
                    vertexs[current_index++].texcoord = Float2(x1, y2);

                    vertexs[current_index].pos = Float3(cx2, cy2, 0.0f);
                    vertexs[current_index].color = text.m_color;
                    vertexs[current_index++].texcoord = Float2(x2, y2);
                    ...
                    vertexs[current_index].pos = Float3(cx2, cy1, 0.0f);
                    vertexs[current_index].color = text.m_color;
                    vertexs[current_index++].texcoord = Float2(x2, y1);
    ...
    void DebugDrawGroup::writeUniformDynamicDataToCache(std::vector<std::pair<Matrix4x4, Float4> >& datas)
    {
        ...
        for (int32_t i = 0; i < 2; i++)
        {
            ...
            for (DebugDrawCapsule capsule : m_capsules)
            {
                if (capsule.m_no_depth_test == no_depth_test)
                {
                    ...
                    tmp.MakeTrans(Float3(0.0f, 0.0f, capsule.m_height / 2.0f - capsule.m_radius));
                    model1 = model1 * tmp;
