...
namespace SE
{
    ...
    {
        return m_descriptor.descriptor_set[RHI::Instance().GetCurrentFrameIndex()];
    }
    ...
    size_t DebugDrawAllocator::cacheUniformDynamicObject(const std::vector<std::pair<Matrix4x4, Float4> >& model_colors)
    {
    ...
    void DebugDrawAllocator::allocator()
    {
        ...
        if (vertex_bufferSize > 0)
        {
            ...
            void* data;
            rhi.MapMemory(m_vertex_resource.memory, 0, vertex_bufferSize, 0, &data);
            memcpy(data, m_vertex_cache.data(), vertex_bufferSize);
            rhi.UnmapMemory(m_vertex_resource.memory);
        }
        ...
        if (uniform_BufferSize > 0)
        {
            ...
            void* data;
            rhi.MapMemory(m_uniform_resource.memory, 0, uniform_BufferSize, 0, &data);
            memcpy(data, &m_uniform_buffer_object.proj_view_matrix, uniform_BufferSize);
            rhi.UnmapMemory(m_uniform_resource.memory);
        }
        ...
        if (uniform_dynamic_BufferSize > 0)
        {
            ...
            void* data;
            rhi.MapMemory(m_uniform_dynamic_resource.memory, 0, uniform_dynamic_BufferSize, 0, &data);
            memcpy(data, m_uniform_buffer_dynamic_object_cache.data(), uniform_dynamic_BufferSize);
            rhi.UnmapMemory(m_uniform_dynamic_resource.memory);
        }
    ...
    void DebugDrawAllocator::flushPendingDelete()
    {
        ...
        while (!m_deffer_delete_queue[current_frame_to_delete].empty())
        {
            ...
            if (resource_to_delete.buffer == nullptr)
                continue;
            rhi.FreeMemory(resource_to_delete.memory);
            rhi.DestroyBuffer(resource_to_delete.buffer);
        }
    }
    ...
    void DebugDrawAllocator::setupDescriptorSet()
    {
        ...
        if (rhi.CreateDescriptorSetLayout(&layoutInfo, m_descriptor.layout) != RHI_SUCCESS)
        {
            throw std::runtime_error("create debug draw layout");
        }

        m_descriptor.descriptor_set.resize(rhi.GetMaxFramesInFlight());
        for (size_t i = 0; i < rhi.GetMaxFramesInFlight(); i++)
        {
            ...
            allocInfo.descriptorPool     = rhi.GetDescriptorPoor();
            allocInfo.descriptorSetCount = 1;
            allocInfo.pSetLayouts = &m_descriptor.layout;

            if (RHI_SUCCESS != rhi.AllocateDescriptorSets(&allocInfo, m_descriptor.descriptor_set[i]))
            {
    ...
    void DebugDrawAllocator::prepareDescriptorSet()
    {
        ...
        image_info[0].sampler     = rhi.GetOrCreateDefaultSampler(Default_Sampler_Linear);
        ...
        for (size_t i = 0; i < rhi.GetMaxFramesInFlight(); i++)
        {
            ...
            rhi.UpdateDescriptorSets(1, descriptor_write, 0, nullptr);
        }
    }
    ...
    void DebugDrawAllocator::updateDescriptorSet()
    {
        ...
        descriptor_write[0].sType            = RHI_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptor_write[0].dstSet           = m_descriptor.descriptor_set[rhi.GetCurrentFrameIndex()];
        ...
        descriptor_write[1].dstSet           = m_descriptor.descriptor_set[rhi.GetCurrentFrameIndex()];
        ...
        rhi.UpdateDescriptorSets(2, descriptor_write, 0, nullptr);
    }
    ...
    void DebugDrawAllocator::loadSphereMeshBuffer()
    {
        ...
        for (int32_t i = -param - 1; i < param + 1; i++)
        {
            ...
            {
                Float3 p(Math::cos(_2pi / (2.0f * param) * j) * r, Math::sin(_2pi / (2.0f * param) * j) * r, h);
                Float3 p1(Math::cos(_2pi / (2.0f * param) * j) * r1, Math::sin(_2pi / (2.0f * param) * j) * r1, h1);
                vertexs[current_index].pos = p;
                vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
            ...
            if (i != -param - 1)
            {
                ...
                {
                    Float3 p(Math::cos(_2pi / (2.0f * param) * j) * r, Math::sin(_2pi / (2.0f * param) * j) * r, h);
                    Float3 p1(Math::cos(_2pi / (2.0f * param) * (j + 1)) * r, Math::sin(_2pi / (2.0f * param) * (j + 1)) * r, h);
                    vertexs[current_index].pos = p;
                    vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
        ...
        void* data;
        rhi.MapMemory(stagingBuffer.memory, 0, bufferSize, 0, &data);
        memcpy(data, vertexs.data(), bufferSize);
        rhi.UnmapMemory(stagingBuffer.memory);

        rhi.CopyBuffer(stagingBuffer.buffer, m_sphere_resource.buffer, 0, 0, bufferSize);

        rhi.DestroyBuffer(stagingBuffer.buffer);
        rhi.FreeMemory(stagingBuffer.memory);
    }
    ...
    void DebugDrawAllocator::loadCylinderMeshBuffer()
    {
        ...
        {
            Float3 p(Math::cos(_2pi / (2.0f * param) * i), Math::sin(_2pi / (2.0f * param) * i), 1.0f);
            Float3 p_(Math::cos(_2pi / (2.0f * param) * (i + 1)), Math::sin(_2pi / (2.0f * param) * (i + 1)), 1.0f);
            Float3 p1(Math::cos(_2pi / (2.0f * param) * i), Math::sin(_2pi / (2.0f * param) * i), -1.0f);
            Float3 p1_(Math::cos(_2pi / (2.0f * param) * (i + 1)), Math::sin(_2pi / (2.0f * param) * (i + 1)), -1.0f);
            ...
            vertexs[current_index].pos = p;
            vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
            vertexs[current_index].pos = Float3(0.0f, 0.0f, 1.0f);
            vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
        ...
        rhi.CopyBuffer(stagingBuffer.buffer, m_cylinder_resource.buffer, 0, 0, bufferSize);
    ...
    void DebugDrawAllocator::loadCapsuleMeshBuffer()
    {
        ...
        for (int32_t i = 0; i < param; i++)
        {
            ...
            for (int32_t j = 0; j < 2 * param; j++)
            {
                Float3 p(Math::cos(_2pi / (2.0f * param) * j) * r, Math::sin(_2pi / (2.0f * param) * j) * r, h + 1.0f);
                Float3 p_(Math::cos(_2pi / (2.0f * param) * (j + 1)) * r, Math::sin(_2pi / (2.0f * param) * (j + 1)) * r, h + 1.0f);
                Float3 p1(Math::cos(_2pi / (2.0f * param) * j) * r1, Math::sin(_2pi / (2.0f * param) * j) * r1, h1 + 1.0f);
                vertexs[current_index].pos = p;
                vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
                vertexs[current_index].pos = p1;
                vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
        ...
        for (int32_t j = 0; j < 2 * param; j++)
        {
            Float3 p(Math::cos(_2pi / (2.0f * param) * j), Math::sin(_2pi / (2.0f * param) * j), 1.0f);
            Float3 p1(Math::cos(_2pi / (2.0f * param) * j), Math::sin(_2pi / (2.0f * param) * j), -1.0f);
            vertexs[current_index].pos = p;
            vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
            vertexs[current_index].pos = p1;
            vertexs[current_index++].color = Float4(1.0f, 0.0f, 0.0f, 1.0f);
        ...
        rhi.CopyBuffer(stagingBuffer.buffer, m_capsule_resource.buffer, 0, 0, bufferSize);
