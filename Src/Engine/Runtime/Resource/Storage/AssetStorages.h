...
namespace SE
{
	...
	/// <summary>
	/// AssetStorages 负责资源数据管理
	/// </summary>
	class SE_API_RUNTIME AssetStorages
	{
	public:
		/// <summary>
	...
	public:
		...
		static StorageReference GetStorage(const StringView& path, bool loadIt = true);
		...
		static StorageReference TryGetStorage(const StringView& path);
		...
		/// <param name="path">The path.</param>
		/// <returns>Flax Storage or null in no package at that location has been opened.</returns>
		static StorageReference EnsureAccess(const StringView& path);
		...
		static bool HasAsset(const UID& id);
		...
		static bool GetAssetEntry(const String& path, Storage::Entry& output);
	...
	public:
		...
		static bool IsStoragePath(const String& path);
		...
		static bool IsStorageExtension(const String& extension);
	...
	public:
		...
		static void GetPackages(List<PackageStorage*>& result);

		/// <summary>
		/// Gets the files.
		/// </summary>
		/// <param name="result">The result.</param>
		static void GetFiles(List<FileStorage*>& result);

		/// <summary>
		/// Gets the storage containers (packages and files).
		/// </summary>
		/// <param name="result">The result.</param>
		static void GetStorage(List<Storage*>& result);
	};
