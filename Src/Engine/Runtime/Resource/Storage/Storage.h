...
namespace SE
{
	...
	/// <summary>
	/// assets storage container.
	/// </summary>
	class SE_API_RUNTIME Storage : public Object
	{
	...
	public:
		...
		struct Entry
		{
			...
			Entry(const UID& id, const TypeID& type, uint32 address)
				: ID(id), Type(type), Address(address)
			{
			}
		};
	...
	protected:
		...
		// Storage
		Threading::ThreadLocal<FileReadStream*> _file;
		List<StorageChunk*> _chunks;
	...
	protected:
		explicit Storage(const StringView& path);

	private:
		// Used by FlaxStorageReference:
		void AddRef()
		{
			Platform::AtomicIncrement(&_refCount);
		}
		void RemoveRef()
		{
			Platform::AtomicDecrement(&_refCount);
	...
	public:
		...
		{
			Platform::AtomicIncrement(&_chunksLock);
		}
		...
		{
			Platform::AtomicDecrement(&_chunksLock);
		}
	...
	public:
		...
		virtual bool HasAsset(const UID& id) const = 0;
		...
		/// </summary>
		/// <param name="output">The output.</param>
		virtual void GetEntries(List<Entry>& output) const = 0;

		/// <summary>
		/// Gets all the chunks in the storage.
		/// </summary>
		/// <param name="output">The output.</param>
		FORCE_INLINE void GetChunks(List<StorageChunk*>& output) const
		{
			output.Add(_chunks);
		}
	...
	public:
		...
		/// <returns>False if cannot load data, otherwise Ture</returns>
		...
		/// <param name="data">The data.</param>
		/// <returns>False if cannot load data, otherwise Ture</returns>
		bool LoadAssetHeader(const UID& id, AssetInitData& data);
		...
		/// <returns>False if cannot load data, otherwise Ture</returns>
		bool LoadAssetChunk(StorageChunk* chunk);
		...
		/// <returns>False if cannot change data, otherwise Ture</returns>
		bool ChangeAssetID(Entry& e, const UID& newId);
#endif
	...
	public:
		...
		/// <param name="customData">Custom options.</param>
		/// <returns>True if cannot create package, otherwise false</returns>
		FORCE_INLINE static bool Create(const StringView& path, const List<AssetInitData>& data, bool silentMode = false, const CustomData* customData = nullptr)
		{
	...
	protected:
		...
		void AddChunk(StorageChunk* chunk);
		virtual void AddEntry(Entry& e) = 0;
		FileReadStream* OpenFile();
		virtual bool GetEntry(const UID& id, Entry& e) = 0;
	};
