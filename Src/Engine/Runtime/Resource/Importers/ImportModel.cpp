...
namespace SE
{
    bool ImportModel::TryGetImportOptions(const StringView& path, Options& options)
    {
        if (FileSystem::FileExists(path))
        {
            ...
            if (tmpFile
                ...
                && (
                    (tmpFile->GetEntry(0).Type == Typeof<Model>() && !tmpFile->LoadAssetHeader(0, data) && data.SerializedVersion >= 4)
                    /*||
                    (tmpFile->GetEntry(0).Type == SkinnedModel::TypeName && !tmpFile->LoadAssetHeader(0, data) && data.SerializedVersion >= 1)
                    ||
                    (tmpFile->GetEntry(0).Type == Animation::TypeName && !tmpFile->LoadAssetHeader(0, data) && data.SerializedVersion >= 1)*/
                ))
            {
                // Check import meta
    ...
    void SetupMaterialSlots(ModelData& data, const List<MaterialSlotEntry>& materials)
    {
        List<int32> materialSlotsTable;
    ...
    bool SortMeshGroups(const IGrouping<StringView, MeshData*>& i1, const IGrouping<StringView, MeshData*>& i2)
    {
    ...
    CreateAssetResult ImportModel::Import(CreateAssetContext& context)
    {
        ...
        else
        {
            ...
            {
                LOG_WARNING("Resource", "Missing model import options. Using default values.");
            }
        }
        ...
        ModelData dataThis;
        List<IGrouping<StringView, MeshData*>>* meshesByNamePtr = options.Cached ? (List<IGrouping<StringView, MeshData*>>*)options.Cached->MeshesByName : nullptr;
        List<IGrouping<StringView, MeshData*>> meshesByNameThis;
        String autoImportOutput;
        if (!data)
        {
            String errorMsg;
            autoImportOutput = FileSystem::GetDirectoryName(context.TargetAssetPath);
            autoImportOutput /= options.SubAssetFolder.HasChars() ? options.SubAssetFolder.TrimTrailing() : String(FileSystem::GetFileNameWithoutExtension(context.InputPath));
            if (!ModelUtils::ImportModel(context.InputPath, dataThis, options, errorMsg, autoImportOutput))
            {
                LOG_ERROR("Resource", "Cannot import model file. {0}", errorMsg);
            ...
            {
                auto f = CreateFunc([](MeshData* const& x) -> StringView
                {
                    return x->Name;
                });
                ListExtensions::GroupBy(data->LODs[0].Meshes, f, meshesByNameThis);

                Sorting::QuickSort(meshesByNameThis.Get(), meshesByNameThis.Count(), CreateFunc(SortMeshGroups));
            }
            meshesByNamePtr = &meshesByNameThis;
        }
        List<IGrouping<StringView, MeshData*>>& meshesByName = *meshesByNamePtr;

        // Import objects from file separately
        ModelUtils::Options::CachedData cached = { data, (void*)meshesByNamePtr };
        List<PrefabObject> prefabObjects;
        if (options.Type == ModelUtils::ModelType::Prefab)
        {
            // Normalize options
            ...
            Function<bool(Options& splitOptions, const StringView& objectName, String& outputPath, MeshData* meshData)> splitImport = [&context, &autoImportOutput](Options& splitOptions, const StringView& objectName, String& outputPath, MeshData* meshData)
            {
                ...
                const int32 splitPos = postFix.FindLast(SE_TEXT('|'));
                if (splitPos != -1)
                    postFix = postFix.Substring(splitPos + 1);
                // TODO: check for name collisions with material/texture assets
                outputPath = autoImportOutput / String(FileSystem::GetFileNameWithoutExtension(context.TargetAssetPath)) + SE_TEXT(" ") + postFix + SE_TEXT(".flax");
                splitOptions.SubAssetFolder = SE_TEXT(" "); // Use the same folder as asset as they all are imported to the subdir for the prefab (see SubAssetFolder usage above)

                if (splitOptions.Type == ModelUtils::ModelType::Model && meshData)
                {
                ...
                return AssetsImporting::Import(context.InputPath, outputPath, &splitOptions);
            };
            auto splitOptions = options;
            LOG_INFO("Resource", "Splitting imported {0} meshes", meshesByName.Count());
            ...
            for (int32 groupIndex = 0; groupIndex < meshesByName.Count(); groupIndex++)
            {
                ...
                splitOptions.Type = ModelUtils::ModelType::Model;
            ...
            /*LOG_INFO("Resource", "Splitting imported {0} animations", data->Animations.Count());
            for (int32 i = 0; i < data->Animations.Count(); i++)
            {
                auto& animation = data->Animations[i];
                splitOptions.Type = ModelUtils::ModelType::Animation;
        ...
        else if (options.SplitObjects)
        {
            ...
            splitImport.Bind([&context](Options& splitOptions, const StringView& objectName)
            {
                ...
                if (splitPos != -1)
                    postFix = postFix.Substring(splitPos + 1);
                const String outputPath = String(FileSystem::GetPathWithoutExtension(context.TargetAssetPath)) + SE_TEXT(" ") + postFix + SE_TEXT(".sge");
                return AssetsImporting::Import(context.InputPath, outputPath, &splitOptions);
            });
            auto splitOptions = options;
            switch (options.Type)
            {
            case ModelUtils::ModelType::Model:
            case ModelUtils::ModelType::SkinnedModel:
                LOG_INFO("Resource", "Splitting imported {0} meshes", meshesByName.Count());
                ...
                break;
            case ModelUtils::ModelType::Animation:
                /*LOG_INFO("Resource", "Splitting imported {0} animations", data->Animations.Count());
        ...
        List<MeshData*> meshesToDelete;
        if (options.ObjectIndex >= 0 &&
            options.ObjectIndex < meshesByName.Count() &&
            (options.Type == ModelUtils::ModelType::Model || options.Type == ModelUtils::ModelType::SkinnedModel))
        {
            ...
            if (&dataThis == data)
            {
                ...
                for (int32 lodIndex = 1; lodIndex < dataThis.LODs.Count(); lodIndex++)
                {
                    ...
                    List<MeshData*> lodMeshes = lod.Meshes;
                    lod.Meshes.Clear();
            ...
            else
            {
                ...
                {
                    List<MeshData*> lodMeshes = data->LODs[lodIndex].Meshes;
        ...
        const bool restoreAnimEvents = false;//RestoreAnimEventsOnReimport && options.Type == ModelUtils::ModelType::Animation && data->Animations.HasItems();
        if ((restoreMaterials || restoreAnimEvents) && FileSystem::FileExists(context.TargetAssetPath))
        {
            AssetRef<Asset> asset = AssetContent::LoadAsync<Asset>(context.TargetAssetPath);
            if (asset && !asset->WaitForLoaded())
            {
                auto* model = asset.As<ModelBase>();
        ...
        /*if (options.Type == ModelUtils::ModelType::Model && options.LightmapUVsSource == ModelLightmapUVsSource::Generate && data->LODs.HasItems() && data->LODs[0].Meshes.Count() > 1)
        {
        ...
        case ModelUtils::ModelType::Animation:
            ...
            break;
        case ModelUtils::ModelType::Prefab:
    ...
    CreateAssetResult ImportModel::Create(CreateAssetContext& context)
    {
        ...
        {
            LOG_WARNING("Resource", "Models has no valid meshes");
            return CreateAssetResult::Error;
        }
    ...
    {
        PROFILE_CPU();
        IMPORT_SETUP(Model, MODE_VERSION);
        ...
        for (int32 lodIndex = 0; lodIndex < lodCount; lodIndex++)
        {
            ...
            for (int32 meshIndex = 0; meshIndex < meshes.Count(); meshIndex++)
            {
                ...
                {
                    LOG_WARNING("Resource", "Cannot pack mesh.");
                    return CreateAssetResult::Error;
                }
            }
        ...
        {
            /*stream.SetPosition(0);
            if (!ModelUtils::GenerateModelSDF(nullptr, &modelData, options->SDFResolution, lodCount - 1, nullptr, &stream, context.TargetAssetPath))
            {
    ...
    CreateAssetResult ImportModel::CreatePrefab(CreateAssetContext& context, ModelData& data, const Options& options, const List<PrefabObject>& prefabObjects)
    {
        PROFILE_CPU();
        ...
        const String outputPath = String(FileSystem::GetPathWithoutExtension(context.TargetAssetPath)) + DEFAULT_PREFAB_EXTENSION_DOT;
        auto* prefab = FileSystem::FileExists(outputPath) ? AssetContent::Load<Prefab>(outputPath) : nullptr;
        if (prefab)
        {
