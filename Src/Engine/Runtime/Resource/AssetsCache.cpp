...
/*		if (version != FLAXENGINE_VERSION_BUILD)
		...
		// Flags
		AssetsCacheFlags flags = AssetsCacheFlags::None;
		stream->ReadInt32((int32*)&flags);

		// Check if other workspace instance used this cache
		if (EnumHasAnyFlags(flags, AssetsCacheFlags::RelativePaths) && enginePath != EngineContext::StartupFolder)
		{
			LOG_WARNING("Resource", "Assets cache generated by the different {1} installation in \'{0}\'", enginePath, SE_TEXT("engine"));
			return;
		}
		if (EnumHasAnyFlags(flags, AssetsCacheFlags::RelativePaths) && projectPath != EngineContext::ProjectFolder)
		{
