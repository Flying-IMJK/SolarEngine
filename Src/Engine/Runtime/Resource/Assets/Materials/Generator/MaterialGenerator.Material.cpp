...
void MaterialGenerator::ProcessGroupMaterial(Box* box, Node* node, Value& value)
{
    ...
    case 8:
    {
        ...
        {
            auto inputName = SE_TEXT("Input") + StringUtils::ToString(i);
        ...
        for (int32 i = 0; i < OutputsMax; i++)
        {
            auto outputName = SE_TEXT("Output") + StringUtils::ToString(i);
            const auto outputBox = node->GetBox(Output0BoxID + i);
            if (outputBox && outputBox->HasConnection())
            {
    ...
    case 20:
    {
        ...
        {
            OnError(node, box, SE_TEXT("Too many VS to PS interpolants used."));
            value = Value::Zero;
            break;
        }
        ...
        {
            OnError(node, box, SE_TEXT("VS to PS interpolants are not supported in Decal or Post Process materials."));
            value = Value::Zero;
            break;
        }
    ...
    case 22:
    {
        ...
        {
            value = Value::One;
            OnError(node, box, SE_TEXT("Invalid terrain layer index."));
            break;
        }
    ...
    {
        /*// Load function asset
        const auto function = Assets.LoadAsync<MaterialFunction>((UID)node->Values[0]);
        if (!function || function->WaitForLoaded())
        {
            OnError(node, box, SE_TEXT("Missing or invalid function."));
            value = Value::Zero;
            break;
        }
        ...
        for (int32 i = _callStack.Count() - 1; i >= 0; i--)
        {
            if (_callStack[i]->Type == GRAPH_NODE_MAKE_TYPE(1, 24))
            {
                ...
                {
                    OnError(node, box, String::Format(SE_TEXT("Recursive call to function '{0}'!"), function->ToString()));
                    value = Value::Zero;
                    return;
                }
            }
        ...
        {
            OnError(node, box, SE_TEXT("Invalid function output box."));
            value = Value::Zero;
            break;
        }
    ...
    case 26:
    {
        ...
        const String text1 = String::Format(SE_TEXT("(float2({0}.xy) + float2({1}.xy) * 2.0)"), baseNormal.Value, additionalNormal.Value);
        const auto appendXY = writeLocal(ValueType::Float2, text1, node);

        const String text2 = String::Format(SE_TEXT("float3({0}, sqrt(saturate(1.0 - dot({0}.xy, {0}.xy))))"), appendXY.Value);
    ...
    case 27:
    {
        ...
        auto x1 = writeLocal(ValueType::Float2, String::Format(SE_TEXT("({0} * -1) + {1}"), center.Value, uv.Value), node);
        auto raCosSin = writeLocal(ValueType::Float2, String::Format(SE_TEXT("float2(cos({0}), sin({0}))"), rotationAngle.Value), node);

        auto dotB1 = writeLocal(ValueType::Float2, String::Format(SE_TEXT("float2({0}.x, {0}.y * -1)"), raCosSin.Value), node);
        auto dotB2 = writeLocal(ValueType::Float2, String::Format(SE_TEXT("float2({0}.y, {0}.x)"), raCosSin.Value), node);

        value = writeLocal(ValueType::Float2, String::Format(SE_TEXT("{3} + float2(dot({0},{1}), dot({0},{2}))"), x1.Value, dotB1.Value, dotB2.Value, center.Value), node);
        break;
    }
    // Sphere Mask
    case 28:
    {
        ...
        auto x1 = writeLocal(ValueType::Float, String::Format(SE_TEXT("distance({0}, {1}) / (float){2}"), a.Value, b.Value, radius.Value), node);

        // Apply hardness, use 0.991 as max since any value above will result in harsh aliasing
        auto x2 = writeLocal(ValueType::Float, String::Format(SE_TEXT("saturate((1 - {0}) * (1 / (1 - clamp({1}, 0, 0.991f))))"), x1.Value, hardness.Value), node);

        value = writeLocal(ValueType::Float, String::Format(SE_TEXT("{0} ? (1 - {1}) : {1}"), invert.Value, x2.Value), node);
        break;
    }
    // Tiling & Offset
    case 29:
    {
        ...
        value = writeLocal(ValueType::Float2, String::Format(SE_TEXT("{0} * {1} + {2}"), uv.Value, tiling.Value, offset.Value), node);
        break;
    }
    // DDX
    case 30:
    {
        if (_treeType == MaterialTreeType::PixelShader)
        {
            ...
            value = writeLocal(inValue.Type, String::Format(SE_TEXT("ddx({0})"), inValue.Value), node);
        }
        else
        {
    ...
    case 31:
    {
        ...
        {
            const auto inValue = tryGetValue(node->GetBox(0), 0, Value::Zero);
            value = writeLocal(inValue.Type, String::Format(SE_TEXT("ddy({0})"), inValue.Value), node);
        }
        else
        {
            // No derivatives support in VS/DS
    ...
    {
        const auto inValue = tryGetValue(node->GetBox(0), 0, Value::Zero);
        value = writeLocal(ValueType::Float, String::Format(SE_TEXT("sign({0})"), inValue.Value), node);
        break;
    }
    // Any
    case 33:
    {
        const auto inValue = tryGetValue(node->GetBox(0), 0, Value::Zero);
        value = writeLocal(ValueType::Bool, String::Format(SE_TEXT("any({0})"), inValue.Value), node);
        break;
    }
    // All
    case 34:
    {
        const auto inValue = tryGetValue(node->GetBox(0), 0, Value::Zero);
        value = writeLocal(ValueType::Bool, String::Format(SE_TEXT("all({0})"), inValue.Value), node);
        break;
    }
    // Blackbody
    case 35:
    {
        ...
        // Value X
        auto x = writeLocal(ValueType::Float, String::Format(SE_TEXT("56100000.0f * pow({0}, -1) + 148.0f"), temperature.Value), node);

        // Value Y
        auto y = writeLocal(ValueType::Float, String::Format(SE_TEXT("{0} > 6500.0f ? 35200000.0f * pow({0}, -1) + 184.0f : 100.04f * log({0}) - 623.6f"), temperature.Value), node);

        // Value Z
        auto z = writeLocal(ValueType::Float, String::Format(SE_TEXT("194.18f * log({0}) - 1448.6f"), temperature.Value), node);

        // Final color
        auto color = writeLocal(ValueType::Float3, String::Format(SE_TEXT("float3({0}, {1}, {2})"), x.Value, y.Value, z.Value), node);
        color = writeLocal(ValueType::Float3, String::Format(SE_TEXT("clamp({0}, 0.0f, 255.0f) / 255.0f"), color.Value), node);
        value = writeLocal(ValueType::Float3, String::Format(SE_TEXT("{1} < 1000.0f ? {0} * {1}/1000.0f : {0}"), color.Value, temperature.Value), node);
        break;
    }
    // HSVToRGB
    case 36:
    {
        ...
        // Normalize from 360
        auto color = writeLocal(ValueType::Float3, String::Format(SE_TEXT("float3({0}.x / 360.0f, {0}.y, {0}.z)"), hsv.Value), node);

        auto x1 = writeLocal(ValueType::Float3, String::Format(SE_TEXT("clamp(abs(fmod({0}.x * 6.0 + float3(0.0f, 4.0f, 2.0f), 6.0f) - 3.0f) - 1.0f, 0.0f, 1.0f)"), color.Value), node);
        value = writeLocal(ValueType::Float3, String::Format(SE_TEXT("{1}.z * lerp(float3(1.0, 1.0, 1.0), {0}, {1}.y)"), x1.Value, color.Value), node);
        break;
    }
    // RGBToHSV
    case 37:
    {
        ...
        const auto epsilon = writeLocal(ValueType::Float, SE_TEXT("1e-10"), node);

        auto p = writeLocal(ValueType::Float4, String::Format(SE_TEXT("({0}.g < {0}.b) ? float4({0}.bg, -1.0f, 2.0f/3.0f) : float4({0}.gb, 0.0f, -1.0f/3.0f)"), rgb.Value), node);
        auto q = writeLocal(ValueType::Float4, String::Format(SE_TEXT("({0}.r < {1}.x) ? float4({1}.xyw, {0}.r) : float4({0}.r, {1}.yzx)"), rgb.Value, p.Value), node);
        auto c = writeLocal(ValueType::Float, String::Format(SE_TEXT("{0}.x - min({0}.w, {0}.y)"), q.Value), node);
        auto h = writeLocal(ValueType::Float, String::Format(SE_TEXT("abs(({0}.w - {0}.y) / (6 * {1} + {2}) + {0}.z)"), q.Value, c.Value, epsilon.Value), node);

        auto hcv = writeLocal(ValueType::Float3, String::Format(SE_TEXT("float3({0}, {1}, {2}.x)"), h.Value, c.Value, q.Value), node);
        value = writeLocal(ValueType::Float3, String::Format(SE_TEXT("float3({0}.x * 360.0f, {0}.y / ({0}.z + {1}), {0}.z)"), hcv.Value, epsilon.Value), node);
        break;
    }
    // View Size
    case 39:
    ...
    case 40:
    {
        ...
        auto d = writeLocal(ValueType::Float2, String::Format(SE_TEXT("abs({0} * 2 - 1) - {1}"), uv.Value, rectangle.Value), node);
        auto d2 = writeLocal(ValueType::Float2, String::Format(SE_TEXT("1 - {0} / fwidth({0})"), d.Value), node);
        value = writeLocal(ValueType::Float, String::Format(SE_TEXT("saturate(min({0}.x, {0}.y))"), d2.Value), node);
        break;
    }
    // FWidth
    case 41:
    {
        const auto inValue = tryGetValue(node->GetBox(0), 0, Value::Zero);
        value = writeLocal(inValue.Type, String::Format(SE_TEXT("fwidth({0})"), inValue.Value), node);
        break;
    }
    // AA Step
    case 42:
    {
        ...
        auto change = writeLocal(ValueType::Float, String::Format(SE_TEXT("fwidth({0})"), gradient.Value), node);

        // Base the range of the inverse lerp on the change over two pixels
        auto lowerEdge = writeLocal(ValueType::Float, String::Format(SE_TEXT("{0} - {1}"), compValue.Value, change.Value), node);
        auto upperEdge = writeLocal(ValueType::Float, String::Format(SE_TEXT("{0} + {1}"), compValue.Value, change.Value), node);

        // Do the inverse interpolation and saturate it
        value = writeLocal(ValueType::Float, String::Format(SE_TEXT("saturate((({0} - {1}) / ({2} - {1})))"), gradient.Value, lowerEdge.Value, upperEdge.Value), node);
    }
    default:
        break;
    }
...
void MaterialGenerator::ProcessGroupFunction(Box* box, Node* node, Value& value)
{
    ...
    case 1:
    {
        ...
        if (!functionCallNode)
        {
            OnError(node, box, SE_TEXT("Missing calling function node."));
            value = Value::Zero;
            break;
        }
        const auto function = Assets.LoadAsync<MaterialFunction>((UID)functionCallNode->Values[0]);
        if (!_functions.TryGet(functionCallNode, graph) || !function)
        {
            OnError(node, box, SE_TEXT("Missing calling function graph."));
            value = Value::Zero;
            break;
        }
        ...
        {
            OnError(node, box, SE_TEXT("Invalid function input box."));
            value = Value::Zero;
            break;
        }
