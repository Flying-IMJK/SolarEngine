...
namespace SE
{
    MaterialValue* MaterialGenerator::sampleTextureRaw(Node* caller, Value& value, Box* box, SerializedMaterialParam* texture)
    {
        ...
        {
            OnError(caller, box, SE_TEXT("No parameter for texture sample node."));
            return nullptr;
        }
        ...
        if (valueBox->Cache.IsInvalid())
        {
            ...
            {
                // Use default UVs
                uv = use3dUVs ? SE_TEXT("float3(input.TexCoord.xy, 0)") : SE_TEXT("input.TexCoord.xy");
            }

            // Select sampler
            // TODO: add option for texture groups and per texture options like wrap mode etc.
            // TODO: changing texture sampler option
            const Char* sampler = SE_TEXT("SamplerLinearWrap");

            // Sample texture
            if (isNormalMap)
            {
                const Char* format = canUseSample ? SE_TEXT("{0}.Sample({1}, {2}).xyz") : SE_TEXT("{0}.SampleLevel({1}, {2}, 0).xyz");
                ...
                _writer.Write(SE_TEXT("\t{0}.z = sqrt(saturate(1.0 - dot({0}.xy, {0}.xy)));\n"), normalVector.Value);
            ...
            else
            {
                ...
                //format = SE_TEXT("SAMPLE_CUBEMAP({0}, {1})");
                }
                else*/
                {
                    /*if (useCustomUVs)
                    ...
                    format = SE_TEXT("SAMPLE_TEXTURE_GRAD({0}, {1}, {2}, {3})");
                    }
                    else*/
                    {
                        format = canUseSample ? SE_TEXT("{0}.Sample({1}, {2})") : SE_TEXT("{0}.SampleLevel({1}, {2}, 0)");
                    }
                }
    ...
    void MaterialGenerator::ProcessGroupTextures(Box* box, Node* node, Value& value)
    {
        ...
        case 1:
        {
            ...
            UID textureId = (UID)node->Values[0];
        ...
        case 17:
        {
            ...
            {
                SE_TEXT("SamplerLinearClamp"),
                SE_TEXT("SamplerPointClamp"),
                SE_TEXT("SamplerLinearWrap"),
                SE_TEXT("SamplerPointWrap"),
            };
            ...
            else if (samplerIndex >= 0 && samplerIndex < ARRAY_SIZE(SamplerNames))
            {
                samplerName = SamplerNames[samplerIndex];
            }
            else
            {
                OnError(node, box, SE_TEXT("Invalid texture sampler."));
                return;
            }
            ...
            if (node->TypeID == 9)
            {
                ...
                {
                    if (useOffset)
                        format = SE_TEXT("{0}.SampleLevel({1}, {2}, {3}, {4})");
                    else
                        format = SE_TEXT("{0}.SampleLevel({1}, {2}, {3})");
                }
                else
                {
                    if (useOffset)
                        format = SE_TEXT("{0}.Sample({1}, {2}, {4})");
                    else
            ...
            else
            {
                ...
                auto proceduralSample = String::Format(SE_TEXT(
                    "   {{\n"
                    "   float3 weights;\n"
        ...
        case 16:
        {
            ...
            const String triplanarTexture = String::Format(SE_TEXT(
                "	{{\n"
