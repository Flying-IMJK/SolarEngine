...
namespace SE
{
	...
	protected:
		...
		List<BinaryAsset*> m_DependantAssets;
		bool m_IsSaving;
	...
	public:
		...
		bool Init(const StorageReference& newStorage, AssetHeader& header);
	...
	protected:
		...
		/// <param name="initData">The initialize data.</param>
		/// <returns>True if cannot init, otherwise false</returns>
		virtual bool OnInit(AssetInitData& initData)
		{
			return true;
		}
	...
	public:
		...
		StorageChunk* GetChunk(int32 index) const
		{
			ENGINE_ASSERT(index >= 0 && index < ASSET_FILE_DATA_CHUNKS);
		...
		StorageChunk* GetOrCreateChunk(int32 index);
		...
		{
			ENGINE_ASSERT(index >= 0 && index < ASSET_FILE_DATA_CHUNKS);
			return m_Header.Chunks[index] != nullptr;
		}
		...
		{
			ENGINE_ASSERT(index >= 0 && index < ASSET_FILE_DATA_CHUNKS);
			return m_Header.Chunks[index] != nullptr ? m_Header.Chunks[index]->LocationInFile.Size : 0;
		}
		...
		{
			ENGINE_ASSERT(index >= 0 && index < ASSET_FILE_DATA_CHUNKS);
			return m_Header.Chunks[index] != nullptr && m_Header.Chunks[index]->IsValid();
		}
		...
		AssetTask* RequestChunkDataAsync(int32 index);
	...
	private:
#if SE_EDITOR
		void OnStorageReloaded(Storage* storage, bool failed);
#endif
	...
	protected:
		// [Asset]
		AssetTask* CreateLoadAssetTask() override;
