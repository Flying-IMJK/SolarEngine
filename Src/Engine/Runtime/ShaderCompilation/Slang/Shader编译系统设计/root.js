(() => {
  ...
  function navigateTo(id, options = {}) {
    ...
    if (local) {
      local.scrollIntoView({ behavior: options.instant ? "auto" : "smooth", block: "start" });
    ...
    // The child may still be loading and may not have reported its exact
    // anchor offsets yet. The manifest lets us move to the correct document
    // immediately, then refine the position when the child responds.
    for (const frame of frames) {
      if (!frameOwnsAnchor(frame, id)) continue;
      scrollToFramePosition(frame, 0, options.instant);
      frame.contentWindow?.postMessage(
        { type: "shader-doc-anchor-request", id },
        "*"
      );
      if (options.updateHash !== false) updateHash(id);
      pendingAnchor = id;
  ...
  window.addEventListener("message", (event) => {
    ...
    if (data.type === "shader-doc-layout") {
      const height = Math.max(120, Math.min(200000, Math.ceil(Number(data.height) || 0)));
      ...
      if (pendingAnchor) {
        requestAnimationFrame(() => navigateTo(pendingAnchor, { instant: true, updateHash: false }));
      }
      return;
    }

    if (
      data.type === "shader-doc-anchor" &&
    ...
    if (data.type === "shader-doc-navigate" && typeof data.id === "string") {
      navigateTo(data.id);
    }
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    // A navigation-tree title is both an expand/collapse control and the
    // entry point of that chapter. Preserve the native details toggle while
    // also navigating to the first anchor in the group.
  ...
  window.addEventListener("hashchange", () => {
    navigateTo(decodeFragment(location.hash.slice(1)), { instant: true, updateHash: false });
  });
  ...
  for (const frame of frames) {
    const requestLayout = () => {
      frame.contentWindow?.postMessage({ type: "shader-doc-request-layout" }, "*");
    };
