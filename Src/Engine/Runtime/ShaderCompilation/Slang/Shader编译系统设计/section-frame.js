(() => {
  ...
  function buildStandaloneNavigation() {
    ...
    group.textContent = document.body.dataset.moduleLabel || "章节";
    nav.append(group);

    for (const section of document.querySelectorAll("main.module-content > section:not([hidden])")) {
  ...
  function initialize() {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (embedded) {
        ...
        window.parent.postMessage({ type: "shader-doc-navigate", id }, "*");
        return;
      }

      // The summary remains a native details toggle. Its click also takes the
      // standalone document to the corresponding chapter.
      if (summary) {
        requestAnimationFrame(() => {
  ...
  window.addEventListener("message", (event) => {
    if (event.data?.type === "shader-doc-request-layout") {
      reportLayout();
      return;
    }

    if (
      event.data?.type === "shader-doc-anchor-request" &&
