...
namespace SE::ShaderParser
{
    ...
    private:
        ...
        void OnParseBefore(IShaderParser* parser, Reader& text) override
        {
            ...
            {
                parser->OnError(SE_TEXT("Invalid constant buffer slot index."));
                return;
            }
            ...
            for (int32 i = 0; i < _cache.Count(); i++)
            {
                ...
                {
                    parser->OnError(String::Format(SE_TEXT("Duplicated constant buffer \'{0}\'. Buffer with that name already exists."), String(_current.Name)));
                    return;
                }
            }
        ...
        void OnParse(IShaderParser* parser, Reader& text) override
        {
            ...
            if (!foundEnd)
            {
                parser->OnError(String::Format(SE_TEXT("Missing constant buffer \'{0}\' ending."), String(_current.Name)));
                return;
            }
        }
        ...
        void CollectResults(IShaderParser* parser, ShaderMeta* result) override
        {
            ...
            for (int32 i = 0; i < _cache.Count(); i++)
            {
                ...
                for (int32 j = i + 1; j < _cache.Count(); j++)
                {
                    ...
                    {
                        parser->OnError(SE_TEXT("Constant buffers slots are overlapping."));
                        return;
                    }
                }
            }
            ...
            for (int32 i = 0; i < _cache.Count(); i++)
            {
                ...
                {
                    parser->OnError(String::Format(SE_TEXT("Constant buffer {0} is using invalid slot {1}. Maximum supported slot is {2}."), String(f.Name), f.Slot, MAX_CONSTANT_BUFFER_SLOTS - 1));
                    return;
                }
            }
