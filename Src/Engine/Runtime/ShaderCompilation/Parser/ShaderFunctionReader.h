...
namespace SE::ShaderParser
{
    ...
    protected:
        ...
        virtual void OnParse(IShaderParser* parser, Reader& text)
        {
            ...
            for (int32 i = 0; i < _cache.Count(); i++)
            {
                ...
                {
                    parser->OnError(String::Format(SE_TEXT("Duplicated function \'{0}\'. Function with that name already exists."), String(_current.Name)));
                    return;
                }
            }
        }
        ...
        void CollectResults(IShaderParser* parser, ShaderMeta* result) override
        {
            ...
            for (int32 i = 0; i < _cache.Count(); i++)
            {
                ...
                for (int32 j = i + 1; j < _cache.Count(); j++)
                {
                    ...
                    {
                        parser->OnError(SE_TEXT("Duplicated shader function names."));
                        return;
                    }
                }
            }
    ...
    public:
        ...
        public:
            ...
            bool CheckStartToken(const Token& token) override
            {
                for (int32 i = 0; i < ARRAY_SIZE(PermutationTokens); i++)
                {
            ...
            void Process(IShaderParser* parser, Reader& text) override
            {
                ...
                ENGINE_ASSERT(_startTokenPermutationSize > 0);
                for (int32 paramIndex = 0; paramIndex < _startTokenPermutationSize; paramIndex++)
                {
                    // Check for missing end
                    if (!text.CanRead())
                    {
                        parser->OnError(SE_TEXT("Missing ending of shader function permutation."));
                        return;
                    }
                    ...
                    if (token.Length == 0)
                    {
                        parser->OnError(SE_TEXT("Incorrect shader permutation. Definition name is empty."));
                        return;
                    }
                    ...
                    if (token.Separator != Separator('='))
                    {
                        ...
                        if (text.PeekChar() != '=')
                        {
                            parser->OnError(SE_TEXT("Incorrect shader permutation. Missing \'='\' character for definition value."));
                            return;
                        }
                    }
                    ...
                    if (token.Length == 0)
                    {
                        parser->OnError(SE_TEXT("Incorrect shader permutation. Definition value is empty."));
                        return;
                    }
                    ...
                    {
                        parser->OnError(SE_TEXT("Incorrect shader permutation declaration."));
                        return;
                    }
                    ...
                    {
                        parser->OnError(String::Format(SE_TEXT("Incorrect shader function permutation definition. Already defined \'{0}\'."), String(name)));
                        return;
                    }
    ...
    protected:
        ...
        void OnParseBefore(IShaderParser* parser, Reader& text) override
        {
            ...
            else
            {
                ...
                current.Flags = ShaderFlags::Hidden;
                //parser->OnError(String::Format(SE_TEXT("Invalid shader function \'isVisible\' option value \'{0}\'."), String(token.ToString())));
                //return;
            }
            text.ReadToken(&token);
            token = parser->GetMacros().GetValue(token);
            ...
            bool missing = true;
            for (int32 i = 0; i < ARRAY_SIZE(levels); i++)
            {
            ...
            if (missing)
            {
                parser->OnError(SE_TEXT("Invalid shader function \'minFeatureLevel\' option value."));
                return;
            }
        ...
        void OnParseAfter(IShaderParser* parser, Reader& text) override
        {
            ...
            {
                parser->OnError(String::Format(SE_TEXT("Function \'{0}\' uses too many permutations. Maximum allowed amount is {1}."), String(current.Name), SHADER_PERMUTATIONS_MAX_COUNT));
                return;
            }
