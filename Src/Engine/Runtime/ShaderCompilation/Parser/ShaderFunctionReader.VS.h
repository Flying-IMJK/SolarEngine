    ...
    class VertexShaderFunctionReader : public ShaderFunctionReader<VertexShaderMeta>
    {
        ...
        public:
            ...
            void Process(IShaderParser* parser, Reader& text) override
            {
                ...
                {
                    parser->OnError(SE_TEXT("Cannot parse token."));
                    return;
                }
                ...
                if (element.Format == PixelFormat::Undefined)
                {
                    parser->OnError(String::Format(SE_TEXT("Unknown input data format \'{0}\' for the Vertex Shader."), String(token.ToString())));
                    return;
                }
                ...
                {
                    parser->OnError(String::Format(SE_TEXT("Invalid input slot class type \'{0}\'."), String(token.ToString())));
                    return;
                }
        ...
        void OnParseAfter(IShaderParser* parser, Reader& text) override
        {
            ...
            {
                parser->OnError(String::Format(SE_TEXT("Vertex Shader \'{0}\' has too many input layout elements specified. Maximum allowed amount is {1}."), String(_current.Name), VERTEX_SHADER_MAX_INPUT_ELEMENTS));
                return;
            }
