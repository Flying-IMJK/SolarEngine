    ...
    class HullShaderFunctionReader : public ShaderFunctionReader<HullShaderMeta>
    {
        ...
        public:
            ...
            void Process(IShaderParser* parser, Reader& text) override
            {
                ...
                {
                    parser->OnError(SE_TEXT("Cannot parse Hull shader input control points count."));
                    return;
                }
                if (Math::RangeExclusive(controlPointsCount, 1, 32))
                {
                    parser->OnError(SE_TEXT("Invalid amount of control points. Valid range is [1-32]."));
                    return;
                }
        ...
        void OnParseAfter(IShaderParser* parser, Reader& text) override
        {
            ...
            {
                parser->OnError(String::Format(SE_TEXT("Hull Shader \'{0}\' has missing META_HS_PATCH macro that defines the amount of the input control points from the Input Assembler."), String(_current.Name)));
                return;
            }
