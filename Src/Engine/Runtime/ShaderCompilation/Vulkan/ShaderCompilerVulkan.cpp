...
namespace SE
{
	const TBuiltInResource* GetDefaultTBuiltInResource();
	...
	{
	private:
		ShaderCompilationContext* m_Context;

		IncludeResult* include(const char* headerName, const char* includerName, int depth) const
		{
			const char* source;
			int32 sourceLength;
			if (!ShaderCompiler::GetIncludedFileSource(m_Context, includerName, headerName, source, sourceLength))
			{
				return nullptr;
			}
	...
	public:
		ShaderIncluder(ShaderCompilationContext* context)
		{
			m_Context = context;
		}
	...
	public:
		Descriptor* Add(glslang::TVarEntryInfo& ent)
		{
			...
			if (DescriptorsCount == SpirvShaderDescriptorInfo::MaxDescriptors)
			{
				...
				LOG_WARNING("ShaderCompiler", "Too many descriptors in use.");
				return nullptr;
			}
			...
			if (type.getBasicType() == glslang::EbtSampler)
			{
				if (!qualifier.hasBinding())
				{
					...
					LOG_WARNING("ShaderCompiler", "Found an uniform \'{0}\' without a binding qualifier. Each uniform must have an explicitly defined binding number.", String(name));
					return nullptr;
				}
				...
				if (type.getSampler().isCombined())
				{
					...
					LOG_WARNING("ShaderCompiler", "Combined sampler \'{0}\' from glsl language is not supported.", String(name));
					return nullptr;
				}
			...
			else if (qualifier.storage == glslang::EvqUniform)
			{
				if (type.getBasicType() != glslang::EbtBlock)
				{
					...
					LOG_WARNING("ShaderCompiler", "Invalid uniform \'{1} {0}\'. Shader uniforms that are not constant buffer blocks are not supported.", String(name), String(type.getBasicTypeString().c_str()));
					return nullptr;
				}
			...
			case VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
				...
				break;
			default:
				LOG_WARNING("ShaderCompiler", "Invalid descriptor type {0} for symbol {1}.", (int32)descriptorType, String(name));
				return nullptr;
			}
	...
	{
		Threading::ScopeLock lock(CompileShaderVulkanLocker);
		if (CompileShaderVulkanInstances == 0)
		{
			glslang::InitializeProcess();
			const auto ver = glslang::GetVersion();
			LOG_INFO("ShaderCompiler", "Using glslang {0}.{1}.{2} compiler (SPIR-V version: {3})", ver.major, ver.minor, ver.patch, String(spvSoftwareVersionString()));
		}
	...
	bool ShaderCompilerVulkan::CompileShader(ShaderFunctionMeta& meta, WritePermutationData customDataWrite)
	{
		...
		Threading::ScopeLock lock(CompileShaderVulkanLocker);
		ShaderIncluder includer(m_Context);

		// Prepare
		if (!WriteShaderFunctionBegin(m_Context, meta))
			return false;

		auto options = m_Context->Options;
		...
		case ShaderStage::Compute:
			...
			break;
		default:
			LOG_ERROR("ShaderCompiler", "Unknown shader type.");
			return false;
		}
		EShMessages messages = (EShMessages)(EShMsgReadHlsl | EShMsgVulkanRules | EShMsgSpvRules );
		...
		for (int32 permutationIndex = 0; permutationIndex < meta.Permutations.Count(); permutationIndex++)
		{
			...
			LOG_WARNING("ShaderCompiler", "VULKAN SHADER {0}: {1}[{2}]", m_Context->Options->TargetName, String(meta.Name), permutationIndex);
#endif
			m_Macros.Clear();

			// Get function permutation macros
			meta.GetDefinitionsForPermutation(permutationIndex, m_Macros);

			// Add additional define for compiled function name
			GetDefineForFunction(meta, m_Macros);

			// Add custom and global macros (global last because contain null define to indicate ending)
			m_Macros.Add(m_Context->Options->Macros);
			m_Macros.Add(m_GlobalMacros);
			...
			const char* names = m_Context->TargetNameAnsi;
			shader.setStringsWithLengthsAndNames(&options->Source, &lengths, &names, 1);
			const int defaultVersion = 450;
			std::string preamble;
			for (int32 i = 0; i < m_Macros.Count() - 1; i++)
			{
...
//			shader.setShiftBinding(glslang::TResourceType::EResUav, 500);
			...
			if (!shader.parse(GetDefaultTBuiltInResource(), defaultVersion, false, messages, includer))
			{
				const auto msg = shader.getInfoLog();
				ShaderCompilationError(m_Context, msg);
				return false;
			}
			...
			ShaderBindings bindings = { 0, 0, 0, 0 };
			{
				...
				for (int blockIndex = 0; blockIndex < program.getNumLiveUniformBlocks(); blockIndex++)
				{
					...
					if (!qualifier.hasBinding())
					{
						...
						LOG_WARNING("ShaderCompiler", "Found a uniform block \'{0}\' without a binding qualifier. Each uniform block must have an explicitly defined binding number.", String(uniform->getTypeName().c_str()));
						continue;
					}
					...
					else
					{
						...
						if (!found)
						{
							LOG_WARNING("ShaderCompiler", "Failed to find descriptor for the uniform block \'{0}\' of size {1} (bytes), binding: {2}.", String(uniform->getTypeName().c_str()), size, binding);
						}
					}
				}
            ...
            for (int32 index = 0; index < program.getNumLiveUniformVariables(); index++)
            {
                ...
                auto qualifier = uniform->getQualifier();
                if (!uniform->isArray())
                    LOG_WARNING("ShaderCompiler", "Shader {0}:{1} - uniform: {2} {3} at binding {4}",
                    m_Context->TargetNameAnsi,
                    String(meta.Name),
				...
				for (int32 i = 0; i < descriptorsCollector.DescriptorsCount; i++)
				{
					...
					if (descriptor.BindingType == SpirvShaderResourceBindingType::CB)
					{
						...
						{
							LOG_WARNING("ShaderCompiler", "Found constant buffer \'{1}\' at slot {0} but it's not used or has no valid size.", descriptor.Slot, String(descriptor.Name.c_str()));
							continue;
						}

						for (int32 b = 0; b < m_ConstantBuffers.Count(); b++)
						{
							auto& cc = m_ConstantBuffers[b];
                ...
                LOG_WARNING("ShaderCompiler", "VULKAN SHADER RESOURCE: slot: {1}, binding: {2}, name: {0}, type: {3}", String(descriptor.Name.c_str()), descriptor.Slot, descriptor.Binding, type);
#endif
				}
			}
			...
			{
				LOG_WARNING("ShaderCompiler", "SPIR-V generator log:\n{0}", String(spirvLogOutput.c_str()));
			}
			if (spirv.empty())
			{
				LOG_WARNING("ShaderCompiler", "SPIR-V generator failed");
				return false;
			}
			...
			// Dump SPIR-V as text for debugging
        {
            ...
            m_Context->OnCollectDebugInfo(meta, permutationIndex, spirvText.c_str(), (int32)spirvText.size());
        }
#endif
			...
			if (!WriteShaderFunctionPermutation(m_Context, meta, permutationIndex, bindings, &header, sizeof(header), &spirv[0], spirvBytesCount))
				return false;

			if (customDataWrite && !customDataWrite(m_Context, meta, permutationIndex, m_Macros))
			{
				return false;
			}
		}

		return WriteShaderFunctionEnd(m_Context, meta);
	}
	...
	bool ShaderCompilerVulkan::OnCompileBegin()
	{
		...
		// glslang compiler adds VULKAN define if EShMsgVulkanRules flag is specified
//		m_GlobalMacros.Add({ "VULKAN", "1" });
