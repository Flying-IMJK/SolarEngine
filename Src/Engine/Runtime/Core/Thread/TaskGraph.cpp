...
/*	TaskGraph::TaskGraph(const SpawnParams& params)
	...
	const List<TaskGraphSystem*, InlinedAllocation<64>>& TaskGraph::GetSystems() const
	{
		return _systems;
	}
	...
	void TaskGraph::Execute()
	{
		...
		while (_remaining.HasItems())
		{
			...
			Function<bool(TaskGraphSystem* const&, TaskGraphSystem* const&)> compare = SortTaskGraphSystem;

			// Execute in order
			Sorting::QuickSort(_queue.Get(), _queue.Count(), compare);
