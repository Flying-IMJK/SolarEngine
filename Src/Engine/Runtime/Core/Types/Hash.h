...
namespace SE
{
	namespace Hash
	{
		...
		namespace XXHash
		{
			SE_API_RUNTIME uint32 GetHash32(void const *pData, uint64 size);

			SE_API_RUNTIME uint32 HashCombine32(void const *pData, uint64 size, uint32 hash);

			SE_API_RUNTIME uint32 GetHash32(List<uint8> &data);

			//-------------------------------------------------------------------------

			SE_API_RUNTIME uint64 GetHash64(void const *pData, uint64 size);

			SE_API_RUNTIME uint64 GetHash64(List<uint8> &data);
		}
    ...
    //-------------------------------------------------------------------------
	inline SE_API_RUNTIME uint32 GetHash(uint64 value)
	{
		return Hash::XXHash::GetHash32(&value, 1);
	}

    inline SE_API_RUNTIME uint32 GetHash(void const *pPtr, uint64 size)
    {
        return Hash::XXHash::GetHash32(pPtr, size);
    }

	inline SE_API_RUNTIME uint32 GetHash(const void* key)
	{
