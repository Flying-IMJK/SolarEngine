...
namespace SE
{
    ...
    public:
        ...
        /// </summary>
        class SE_API_RUNTIME SortingStack
        {
		public:
    ...
    public:
        ...
        inline static void QuickSort(List<T, AllocationType> &data)
        {
		...
		/// </summary>
		/// <param name="data">The data container.</param>
		template<typename T, typename AllocationType = HeapAllocation, typename Func = bool(const T&, const T&)>
		inline static void QuickSort(List<T, AllocationType> &data, Func compare)
		{
			QuickSort(data.Get(), data.Count(), compare);
		}
        ...
        static void QuickSort(T *data, int32 count, Func compare)
        {
            if (count < 2)
                return;
            auto &stack = SortingStack::Get();
        ...
        static void SortArray(T *data, int32 count, Function<bool (const T&, const T&, U *)> compare, U *userData)
        {
            if (count < 2)
                return;
            auto &stack = SortingStack::Get();
        ...
        static void MergeSort(T *data, int32 count, T *tmp = nullptr)
        {
            ...
            if (alloc)
                tmp = (T *) PlatformAllocator::Allocate(sizeof(T) * count, 16);
            MergeSort(data, tmp, 0, count - 1);
            if (alloc)
                PlatformAllocator::Free(tmp);
        }

        template<typename T, typename AllocationType = HeapAllocation>
        inline static void MergeSort(List <T, AllocationType> &data, List <T, AllocationType> *tmp = nullptr)
        {
            if (tmp)
