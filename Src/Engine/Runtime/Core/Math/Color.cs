...
namespace SE
{
    ...
    public partial struct Color
    {
        ...
        public static bool FromHex(string hex, out Color color)
        {
            ...
            {
                int rDigit = HexDigit(hex[startIndex++]);
                int gDigit = HexDigit(hex[startIndex++]);
                int bDigit = HexDigit(hex[startIndex]);
                isValid = rDigit >= 0 && gDigit >= 0 && bDigit >= 0;
                r = (rDigit << 4) + rDigit;
                g = (gDigit << 4) + gDigit;
                b = (bDigit << 4) + bDigit;
            }
            ...
            {
                isValid = TryHexByte(hex, startIndex + 0, out r) &&
                          TryHexByte(hex, startIndex + 2, out g) &&
                          TryHexByte(hex, startIndex + 4, out b);
            }
            else if (hex.Length == 8 + startIndex)
            {
                isValid = TryHexByte(hex, startIndex + 0, out r) &&
                          TryHexByte(hex, startIndex + 2, out g) &&
                          TryHexByte(hex, startIndex + 4, out b) &&
                          TryHexByte(hex, startIndex + 6, out a);
            }
            else
            {
                r = g = b = 0;
            ...
            color = isValid ? new Color(r, g, b, a) : Black;
            return isValid;
        }
        ...
        public static bool TryParseHex(string hexString, out Color value)
        {
            ...
            {
                if (!TryHexByte(hexString, startIndex + 0, out r) ||
                    !TryHexByte(hexString, startIndex + 2, out g) ||
                    !TryHexByte(hexString, startIndex + 4, out b))
                {
                    return false;
                }
            }
            else if (hexString.Length == 8 + startIndex)
            {
                if (!TryHexByte(hexString, startIndex + 0, out r) ||
                    !TryHexByte(hexString, startIndex + 2, out g) ||
                    !TryHexByte(hexString, startIndex + 4, out b) ||
                    !TryHexByte(hexString, startIndex + 6, out a))
                {
                    return false;
                }
            }
        ...
        private static bool TryHexByte(string text, int startIndex, out int value)
        {
            int high = HexDigit(text[startIndex]);
            int low = HexDigit(text[startIndex + 1]);
            value = (high << 4) + low;
            return high >= 0 && low >= 0;
        }
