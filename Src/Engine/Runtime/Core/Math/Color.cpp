...
namespace SE
{
	...
	Float4 Color32::ToFloat4() const
	{
        return Float4(static_cast<float>(r) / 255, static_cast<float>(g) / 255, static_cast<float>(b) / 255, static_cast<float>(a) / 255);
    }

	Color32::operator Vector4Base<float>() const
	{
        return ToFloat4();
    }

	Color32 Color32::GetAlphaVersion(float alpha) const
	{
		float const floatAlpha = Math::Clamp(alpha * 255, 0.0f, 255.0f);
		return GetAlphaVersion(static_cast<uint8>(floatAlpha));
	}

	Color32 Color32::GetScaledColor(float multiplier) const
	{
		Color32 c = *this;
	...
	void Color32::ScaleColor(float multiplier)
	{
