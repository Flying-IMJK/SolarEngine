...
namespace SE
{
    ...
    /// </summary>
    struct SE_API_RUNTIME ScriptingTypeHandle
    {
        BinaryModule* Module;
    ...
    FORCE_INLINE uint32 GetHash(const ScriptingTypeHandle& key)
    {
    ...
    /// </summary>
    struct SE_API_RUNTIME ScriptingType
    {
    ...
    struct SE_API_RUNTIME ScriptingTypeInitializer : ScriptingTypeHandle
    {
        ...
        ScriptingTypeInitializer(BinaryModule* module, const StringAnsiView& fullname, int32 size, StableID stableId, ScriptingType::EnumItem* items);
    ...
    class ScriptingTypeCache
    {
        template<typename Type> friend ScriptingTypeHandle GetScriptingType();
        friend ScriptingTypeInitializer;
    private:
        static Dictionary<StableID, ScriptingTypeInitializer> scriptTypeDict;
    };
    ...
    {
        ScriptingTypeHandle handle;
        if constexpr (TIsEnum<Type>::Value)
        {
            ENGINE_ASSERT(ScriptingTypeCache::scriptTypeDict.TryGet(StableID::Generate<Type>(), handle));
        }
        else
        {
            handle = Type::TypeInitializer;
        }
    ...
    /// </summary>
    extern "C" SE_API_RUNTIME ManagedBinaryModule* GetBinaryModuleCorlib();
