...
namespace SE
{
	...
	class SE_API_RUNTIME FileBase
	{
		NON_COPYABLE(FileBase)
	public:
	...
	public:
		...
		static bool ReadAllBytes(const StringView& path, List<byte, HeapAllocation>& data);
		...
		static bool WriteAllBytes(const StringView& path, const List<byte, HeapAllocation>& data);
		static bool WriteAllText(const StringView& path, const String& data, Encoding::EncodingType encoding);
		static bool WriteAllText(const StringView& path, const StringBuilder& data, Encoding::EncodingType encoding);
		static bool WriteAllText(const StringView& path, const Char* data, int32 length, Encoding::EncodingType encoding);
	};
}