...
#include "Runtime/Core/Platform/Base/DragDropHelper.h"
...
//			Platform::Fatal(TEXT("Cannot create window."));
		...
		if (user32Dll)
		{
			...
			{
				m_Dpi = getDpiForWindowProc(_handle);
			}
			FreeLibrary(user32Dll);
		}
		m_DpiScale = (float)m_Dpi / (float)DefaultDPI;
	
	#if WINDOWS_USE_NEWER_BORDER_LESS
		// Enable shadow
		if (settings.IsRegularWindow && !settings.HasBorder && IsCompositionEnabled())
		{
		...
		if (settings.AllowDragAndDrop)
		{
			...
			if (result != S_OK)
			{
				LOG_WARNING("Platform", "Window drag and drop service error: 0x{0:x}:{1}", result, 1);
			}
		}
	#endif
	...
	WindowsWindow::~WindowsWindow()
	{
		if (HasHWND())
		{
			...
			{
				LOG_WARNING("Platform", "DestroyWindow failed! Error: {0:#x}", GetLastError());
			}
	...
	void WindowsWindow::Show()
	{
		if (!m_Visible)
		{
			...
			// Show
			ShowWindow(_handle, (m_Settings.AllowInput && m_Settings.ActivateWhenFirstShown) ? SW_SHOW : SW_SHOWNA);
#if WINDOWS_USE_NEW_BORDER_LESS
			if (!m_Settings.HasBorder && m_Settings.IsRegularWindow)
			{
	...
	void WindowsWindow::SetBorderless(bool isBorderless, bool maximized)
	{
		...
		m_Settings.HasBorder = !isBorderless;
	...
	void WindowsWindow::SetOpacity(const float opacity)
	{
		...
		SetLayeredWindowAttributes(_handle, 0, static_cast<BYTE>(Math::Clamp(opacity, 0.0f, 1.0f) * 255), LWA_ALPHA);
	}
	...
	void WindowsWindow::StartTrackingMouse(bool useMouseScreenOffset)
	{
		...
		if (!m_IsTrackingMouse)
		{
			...
			m_TrackingMouseOffset = Float2::Zero;
			m_IsUsingMouseOffset = useMouseScreenOffset;
			m_IsHorizontalFlippingMouse = false;
			m_IsVerticalFlippingMouse = false;
	
			int32 x = 0, y = 0, width = 0, height = 0;
			GetScreenInfo(x, y, width, height);
			m_MouseOffsetScreenSize = Rectangle((float)x, (float)y, (float)width, (float)height);
	...
	{
		if (m_IsClippingCursor)
		{
			m_IsClippingCursor = false;
	...
	void WindowsWindow::UpdateCursor()
	{
		...
		if (m_Cursor == CursorType::Hidden && m_Focused)
		{
		...
		ENGINE_ASSERT(index >= 0 && index < ARRAY_SIZE(cursors));
	...
	Windows::LRESULT WindowsWindow::WndProc(Windows::UINT msg, Windows::WPARAM wParam, Windows::LPARAM lParam)
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			if (m_IsTrackingMouse && m_IsUsingMouseOffset)
			{
				// Check if move mouse to another edge of the desktop
				Float2 desktopLocation = m_MouseOffsetScreenSize.Location;
				Float2 desktopSize = m_MouseOffsetScreenSize.GetBottomRight();
		...
		{
#if WINDOWS_USE_NEW_BORDER_LESS
			if (wParam && !m_Settings.HasBorder && m_Settings.IsRegularWindow)
			{
				if (_maximized)
				{
...
#elif WINDOWS_USE_NEWER_BORDER_LESS
			if (wParam == TRUE && !m_Settings.HasBorder) // && m_Settings.IsRegularWindow)
			{
		...
		case WM_NCACTIVATE:
		{
			...
			if (!m_Settings.HasBorder && !IsCompositionEnabled())
				return 1;
			break;
		}
#endif
		...
		case WM_GETMINMAXINFO:
		{
			...
			if (!IsFullscreen() && ((GetWindowPlacement(_handle, &e) && (e.showCmd == SW_SHOWMAXIMIZED || e.showCmd == SW_SHOWMINIMIZED)) || _isDuringMaximize))
			{
				...
				if (monitor != nullptr)
				{
					...
					if (::GetMonitorInfoW(monitor, &monitorInfo))
					{
						minMax->ptMaxPosition.x = Math::Abs((int)monitorInfo.rcWork.left - (int)monitorInfo.rcMonitor.left);
						minMax->ptMaxPosition.y = Math::Abs((int)monitorInfo.rcWork.top - (int)monitorInfo.rcMonitor.top);
						minMax->ptMaxSize.x = Math::Abs((int)monitorInfo.rcWork.right - (int)monitorInfo.rcWork.left);
						minMax->ptMaxSize.y = Math::Abs((int)monitorInfo.rcWork.bottom - (int)monitorInfo.rcWork.top);
					}
				}
			}
		...
		case WM_SETFOCUS:
			OnGotFocus();
			UpdateCursor();
			if (m_IsClippingCursor && !_clipCursorSet)
			{
				_clipCursorSet = true;
				ClipCursor((RECT*)_clipCursorRect);
			}
		...
		case WM_KILLFOCUS:
			...
			break;
		case WM_ACTIVATEAPP:
			if (wParam == TRUE && !m_Focused)
			{
				OnGotFocus();
			}
			else if (wParam == FALSE && m_Focused)
			{
				OnLostFocus();
		...
		case WM_SYSKEYDOWN:
			if (wParam == VK_F4)
			{
				LOG_INFO("Platform", "Alt+F4 pressed");
				Close(ClosingReason::User);
				return 0;
			}
			if (wParam == VK_RETURN)
			{
				LOG_INFO("Platform", "Alt+Enter pressed");
		...
		case WM_DESTROY:
		{
			...
			if (m_Settings.AllowDragAndDrop)
			{
				const auto result = RevokeDragDrop(_handle);
				if (result != S_OK)
					LOG_INFO("Window", "Window drag and drop service error: 0x{0:x}:{1}", result, 2);
			}
#endif
	...
	public:
		...
		void GetAsFiles(List<String>* files) const override
		{
			if (_type == Type::Files)
			{
				files->Add(_data);
	...
	DragDropEffect WindowsWindow::DoDragDrop(const StringView& data)
	{
		...
		Threading::Task::StartNew(task);
		while (task->GetState() == Threading::TaskState::Queued)
			Platform::Sleep(1);
