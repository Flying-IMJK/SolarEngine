...
namespace SE
{
	...
	WindowsGamepad::WindowsGamepad(Windows::DWORD userIndex)
		: Gamepad(UID(0, 0, 0, userIndex), SE_TEXT("XInput Gamepad"))
	...
	void WindowsGamepad::SetVibration(const GamepadVibrationState& state)
	{
		...
		const float leftMotor = Math::Clamp(state.LeftLarge + state.LeftSmall, 0.0f, 1.0f) * 0.5f;
		const float rightMotor = Math::Clamp(state.RightLarge + state.RightSmall, 0.0f, 1.0f) * 0.5f;
