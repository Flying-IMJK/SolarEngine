...
#include "Runtime/Core/Platform/CreateProcessSettings.h"
#include "Runtime/Core/Platform/MemoryStats.h"
#include "Runtime/Core/Types/Delegate.h"
//#include "Core/Platform/Base/PlatformUtils.h"
...
namespace SE
{
	...
	const Char* WindowsPlatform::ApplicationWindowClass = SE_TEXT("SEWindow");
	...
	void GetWindowsVersion(String& windowsName, int32& versionMajor, int32& versionMinor, int32& versionBuild)
	{
		...
		HKEY hKey;
		LONG lRes = RegOpenKeyExW(HKEY_LOCAL_MACHINE, SE_TEXT("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion"), 0, KEY_READ, &hKey);
		if (lRes == ERROR_SUCCESS)
		{
			GetStringRegKey(hKey, SE_TEXT("ProductName"), windowsName, SE_TEXT("Windows"));

			DWORD currentMajorVersionNumber;
			DWORD currentMinorVersionNumber;
			String currentBuildNumber;
			GetDWORDRegKey(hKey, SE_TEXT("CurrentMajorVersionNumber"), currentMajorVersionNumber, 0);
			GetDWORDRegKey(hKey, SE_TEXT("CurrentMinorVersionNumber"), currentMinorVersionNumber, 0);
			GetStringRegKey(hKey, SE_TEXT("CurrentBuildNumber"), currentBuildNumber, SE_TEXT("0"));
			...
			if (StringUtils::Compare(windowsName.Get(), SE_TEXT("Windows 7"), 9) == 0)
			{
				VersionMajor = 6;
				VersionMinor = 2;
			}
			...
			{
				// Windows 11
				windowsName.Replace(SE_TEXT("10"), SE_TEXT("11"));
			}
			else if (VersionMajor == 0 && VersionMinor == 0)
			{
				String windowsVersion;
				GetStringRegKey(hKey, SE_TEXT("CurrentVersion"), windowsVersion, SE_TEXT(""));
		...
		else
		{
			if (IsWindowsServer())
			{
				windowsName = SE_TEXT("Windows Server");
				versionMajor = 6;
				versionMinor = 3;
			}
			else if (IsWindows8Point1OrGreater())
			{
				windowsName = SE_TEXT("Windows 8.1");
				versionMajor = 6;
				versionMinor = 3;
			}
			else if (IsWindows8OrGreater())
			{
				windowsName = SE_TEXT("Windows 8");
				versionMajor = 6;
				versionMinor = 2;
			}
			else if (IsWindows7SP1OrGreater())
			{
				windowsName = SE_TEXT("Windows 7 SP1");
				versionMajor = 6;
				versionMinor = 2;
			}
			else if (IsWindows7OrGreater())
			{
				windowsName = SE_TEXT("Windows 7");
				versionMajor = 6;
				versionMinor = 1;
			}
			else if (IsWindowsVistaSP2OrGreater())
			{
				windowsName = SE_TEXT("Windows Vista SP2");
				versionMajor = 6;
				versionMinor = 1;
			}
			else if (IsWindowsVistaSP1OrGreater())
			{
				windowsName = SE_TEXT("Windows Vista SP1");
				versionMajor = 6;
				versionMinor = 1;
			}
			else if (IsWindowsVistaOrGreater())
			{
				windowsName = SE_TEXT("Windows Vista");
				versionMajor = 6;
				versionMinor = 0;
			}
		}
	...
	LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		if (hwnd != nullptr)
		{
			...
			if (win)
			{
				return reinterpret_cast<WindowsWindow*>(win)->WndProc(msg, wParam, lParam);
			}
		}
	...
	long __stdcall WindowsPlatform::SehExceptionHandler(EXCEPTION_POINTERS* ep)
	{
		...
		// Skip if engine already crashed TODO
/*		if (Globals::FatalErrorOccurred)
		...
		errorMsg.Append("Unhandled exception: ");

		switch (ep->ExceptionRecord->ExceptionCode)
		{
#define CASE(x) case x: errorMsg.Append(#x); break;
...
#undef CASE
		case EXCEPTION_ACCESS_VIOLATION:
			errorMsg.Append("EXCEPTION_ACCESS_VIOLATION ");
			if (ep->ExceptionRecord->ExceptionInformation[0] == 0)
			{
				errorMsg.Append("reading address ");
			}
			else if (ep->ExceptionRecord->ExceptionInformation[0] == 1)
			{
				errorMsg.Append("writing address ");
			}
			errorMsg.Append(String::Format(SE_TEXT("{:#x}"), (uint32)ep->ExceptionRecord->ExceptionInformation[1]));
			break;
		default:
			errorMsg.Append(String::Format(SE_TEXT("{:#x}"), (uint32)ep->ExceptionRecord->ExceptionCode));
		}

		// Log exception and return to the crash location when using debugger
		if (Platform::IsDebuggerPresent())
		{
			LOG_ERROR("Platform", "{0}", errorMsg.ToString());
			const String stackTrace = Platform::GetStackTrace(0, 60, ep);
			LOG_ERROR("Platform", "{0}", stackTrace);
			return EXCEPTION_CONTINUE_SEARCH;
		}
...
//		Platform::Fatal(errorMsg.Get(), ep);
	...
	bool GetModuleListPSAPI(HANDLE hProcess)
	{
		...
		HINSTANCE psapiDll = LoadLibraryW(SE_TEXT("psapi.dll"));
		if (psapiDll == nullptr)
			return true;
	...
	DialogResult MessageBox::Show(Window* parent, StringView text, StringView caption, MessageBoxButtons buttons, MessageBoxIcon icon)
	{
	...
	void WindowsPlatform::PreInit(void* hInstance, int icon)
	{
		ASSERT(hInstance);
		Instance = hInstance;
		...
		windowsClass.hIcon = LoadIconW(GetModuleHandle(nullptr), MAKEINTRESOURCE(icon));
		...
		{
			LOG_ERROR("Platform", "Window class registration failed!");
			exit(-1);
		}
		...
		{
			LOG_ERROR("Platform", "OLE initalization failed!");
			exit(-1);
		}
		...
		if (::GetModuleFileNameW(::GetModuleHandleW(nullptr), buffer, MAX_PATH))
			SymbolsPath.Add(FileSystem::GetDirectoryName(buffer));
		if (::GetEnvironmentVariableW(SE_TEXT("_NT_SYMBOL_PATH"), buffer, MAX_PATH))
			SymbolsPath.Add(FileSystem::GetDirectoryName(buffer));
		...
		if (VersionMajor < 6)
		{
			Error(SE_TEXT("Not supported operating system version."));
			exit(-1);
		}
	}
...
/*		if (CommandLine::Options.Std)
		...
		{
//			Platform::Fatal(SE_TEXT("Flax Engine requires Windows Vista SP1 or higher."));
			return true;
		}
	...
	void WindowsPlatform::LogInfo()
	{
		...
		LOG_INFO("Platform", "Microsoft {0} {1}-bit ({2}.{3}.{4})", WindowsName, (Platform::Is64BitPlatform() ? SE_TEXT("64") : SE_TEXT("32")), VersionMajor, VersionMinor, VersionBuild);
		...
		{
			auto msg = String::Format(SE_TEXT("Not enough RAM memory for good application performance.\nDetected: {0} MB\nRecommended : {1} MB\nDo you want to continue ?"), mb, mbMinimum);
			if (MessageBoxW(nullptr, *msg, SE_TEXT("Warning"), MB_ICONWARNING | MB_YESNO) == IDNO)
			{
				LOG_WARNING("Platform", "Not enough RAM. Closing...");
				exit(0);
			}
		}
	}
...
//		WindowsInput::Update();
	...
	void WindowsPlatform::Log(const StringView& msg)
	{
		Char buffer[512];
		Char* str;
		if (msg.Length() + 3 < ARRAY_SIZE(buffer))
			str = buffer;
		else
...
/*	BatteryInfo WindowsPlatform::GetBatteryInfo()
	...
	Rectangle WindowsPlatform::GetVirtualDesktopBounds()
	{
		if (VirtualScreenBounds.Size.x == 0)
		{
			...
			VirtualScreenBounds = Rectangle(Float2(static_cast<float>(screenRect.left),
				static_cast<float>(screenRect.top)),
				Float2(static_cast<float>(screenRect.right - screenRect.left),
				static_cast<float>(screenRect.bottom - screenRect.top)));
		}
	...
	void ReadPipe(HANDLE pipe, List<char>& rawData, List<Char>& logData, Log::Severity logType, CreateProcessSettings& settings)
	{
...
//					Log::Logger::Write(logType, StringView(logData.Get(), rawData.Count()));
	...
	{
		LOG_INFO("Platform", "Command: {0} {1}", settings.FileName, settings.Arguments);
		if (settings.WorkingDirectory.HasChars())
		{
			LOG_INFO("Platform", "Working directory: {0}", settings.WorkingDirectory);
		}
		...
		if (settings.ShellExecute)
		{
			...
			{
				result = 1;
//				LOG_WARNING("Platform", "Cannot start process. Error code: 0x{0:x}", (uint64)GetLastError());
			}
		...
		{
			result = -1;
			const String cmdLine = settings.FileName + SE_TEXT(" ") + settings.Arguments;
			...
			if (settings.Environment.HasItems())
			{
				...
				environmentStr = (Char*)PlatformAllocator::Allocate(totalLength * sizeof(Char));
			...
			if (captureStdOut)
			{
				...
				{
					LOG_WARNING("Platform", "CreatePipe failed");
					return 1;
				}
				...
				{
					startupInfoEx.lpAttributeList = (LPPROC_THREAD_ATTRIBUTE_LIST)PlatformAllocator::Allocate(bufferSize);
					if (!InitializeProcThreadAttributeList(startupInfoEx.lpAttributeList, 1, 0, &bufferSize))
					{
						LOG_WARNING("Platform", "InitializeProcThreadAttributeList failed");
						goto ERROR_EXIT;
					}
				}
				...
				{
					LOG_WARNING("Platform", "UpdateProcThreadAttribute failed");
					goto ERROR_EXIT;
				}
			}
			...
			if (!CreateProcessW(nullptr, const_cast<LPWSTR>(cmdLine.GetText()), nullptr, nullptr, TRUE, dwCreationFlags, (LPVOID)environmentStr, settings.WorkingDirectory.HasChars() ? settings.WorkingDirectory.Get() : nullptr,
				reinterpret_cast<LPSTARTUPINFOW>(&startupInfoEx.StartupInfo), &procInfo))
			{
				LOG_WARNING("Platform", "Cannot start process. Error code: 0x{0:x}", (uint64)GetLastError());
				goto ERROR_EXIT;
			}
			...
			if (stdOutRead != nullptr)
			{
				...
				do
				{
					ReadPipe(stdOutRead, rawData, logData, Log::Severity::Info, settings);
					ReadPipe(stdErrRead, rawData, logData, Log::Severity::Error, settings);
					Sleep(1);
		...
		ERROR_EXIT:
			...
			if (startupInfoEx.lpAttributeList != nullptr)
			{
				...
				PlatformAllocator::Free(startupInfoEx.lpAttributeList);
			}
			if (environmentStr)
				PlatformAllocator::Free(environmentStr);
		}
...
/*	Window* WindowsPlatform::CreateWindow(const CreateWindowSettings& settings)
	...
	{
		ENGINE_ASSERT(filename);

		// Add folder to search path to load dependency libraries
		StringView folder = FileSystem::GetDirectoryName(filename);
		...
		if (!handle)
		{
			LOG_WARNING("Platform", "Failed to load '{0}' (GetLastError={1})", filename, GetLastError());
		}
	...
	List<PlatformBase::StackFrame> WindowsPlatform::GetStackFrames(int32 skipCount, int32 maxDepth, void* context)
	{
		...
		int32 count;
		void* backtrace[100];
		maxDepth = Math::Min<int32>(maxDepth, ARRAY_SIZE(backtrace));
		if (context)
		{
			auto& ctx = *(((EXCEPTION_POINTERS*)context)->ContextRecord);
			STACKFRAME64 stack;
			MemorySet(&stack, 0, sizeof(stack));
			DWORD imageType;
	#ifdef _M_IX86
	...
	#error "Platform not supported!"
		...
		for (int32 i = 0; i < count; i++)
		{
			...
			// Get function name
			alignas(IMAGEHLP_SYMBOL64) byte symbolData[sizeof(IMAGEHLP_SYMBOL64) + ARRAY_SIZE(frame.FunctionName)];
			IMAGEHLP_SYMBOL64* symbol = (IMAGEHLP_SYMBOL64*)symbolData;
			symbol->SizeOfStruct = sizeof(IMAGEHLP_SYMBOL64);
			symbol->MaxNameLength = ARRAY_SIZE(frame.FunctionName) - 1;
			DWORD64 displacement = 0;

			if (SymGetSymFromAddr64(process, (DWORD64)frame.ProgramCounter, &displacement, symbol))
			{
				UnDecorateSymbolName(symbol->Name, frame.FunctionName, ARRAY_SIZE(frame.FunctionName), UNDNAME_COMPLETE);
			}
			...
			if (SymGetLineFromAddr64(process, (DWORD64)frame.ProgramCounter, &offset, &line))
			{
				...
				const int32 fileNameLength = Math::Min<int32>(ARRAY_SIZE(frame.FileName) - 1, StringUtils::Length(line.FileName));
			...
			{
				const int32 moduleNameLength = Math::Min<int32>(ARRAY_SIZE(frame.ModuleName) - 1, StringUtils::Length(module.ImageName));
	...
	void WindowsPlatform::CollectCrashData(const String& crashDataFolder, void* context)
	{
		...
		crashInfo.MiniDumpPath = crashDataFolder / SE_TEXT("Minidump.dmp");
