...
#include "Runtime/Core/Platform/Windows/ComPtr.h"
...
namespace SE
{
	...
	bool WindowsFileSystem::ShowOpenFileDialog(const StringView& initialDirectory, const StringView& filter, bool multiSelect, const StringView& title, List<String, HeapAllocation>& filenames)
	{
		return ShowOpenFileDialog(Window::mainWindow, initialDirectory, filter, multiSelect, title, filenames);
	}

	bool WindowsFileSystem::ShowOpenFileDialog(Window* parentWindow, const StringView& initialDirectory, const StringView& filter, bool multiSelect, const StringView& title, List<String, HeapAllocation>& filenames)
	{
		bool result = false;

		// Allocate memory for the filenames
		const int32 maxFilenamesSize = (multiSelect ? 200 : 2) * MAX_PATH;
		List<Char> fileNamesBuffer;
	...
	bool WindowsFileSystem::ShowSaveFileDialog(Window* parentWindow, const StringView& initialDirectory, const StringView& filter, bool multiSelect, const StringView& title, List<String, HeapAllocation>& filenames)
	{
		bool result = true;
	...
	bool WindowsFileSystem::ShowBrowseFolderDialog(Window* parentWindow, const StringView& initialDirectory, const StringView& title, String& path)
	{
		...
		const UID folderGuid(0x53890ed9, 0xa55e47ba, 0xa970bdae, 0x72acedff);
