...
//#include "Core/Platform/PlatformUtils_Win32.h"
...
namespace SE
{
    ...
    enum class WindowStyle : DWORD
    {
        ...
        BasicBorderless = WS_POPUP | WS_THICKFRAME | WS_CAPTION | WS_SYSMENU | WS_MAXIMIZEBOX | WS_MINIMIZEBOX
    };
    ...
    {
		MessageBoxW(GetActiveWindow(), error.Get(), L"Fatal Error Occurred!", MB_OK | MB_ICONERROR);
        return false;
    }
    ...
    {
        ENGINE_ASSERT((m_windowRect.right - m_windowRect.left) > 0);
        ENGINE_ASSERT((m_windowRect.bottom - m_windowRect.top) > 0);
		...
		m_windowClass.lpszClassName = m_applicationName.Get();
...
//        Platform::SetMainWindowHandle(m_windowHandle);
        ...
        case WM_NCCALCSIZE:
        {
            if (wParam == TRUE && m_isBorderLess)
            {
                ...
                else // Offset the hit-test borders by the system border metric to replicate Windows10+ resize behavior
                {
                    ...
                    int32 const borderWidthX = ::GetSystemMetrics(SM_CXFRAME) + ::GetSystemMetrics(SM_CXPADDEDBORDER);
                    requestedClientArea.right -= borderWidthX;
                    requestedClientArea.left += borderWidthX;

                    int32 const borderWidthY = ::GetSystemMetrics(SM_CYFRAME) + ::GetSystemMetrics(SM_CXPADDEDBORDER);
    ...
    {
        String const layoutIniFilePath = String::Empty;
			/*Platform::GetCurrentProcessId() ::GetCurrentProcessPath() + m_applicationNameNoWhitespace + ".layout.ini" */;
    ...
    void Win32Application::WriteLayoutSettings()
    {
        ...
        if (layoutIni.IsValid())
        {
            ...
            String const layoutIniFilePath = String::Empty;
				//FileSystem::GetCurrentProcessPath() + m_applicationNameNoWhitespace + ".layout.ini";
    ...
    int32 Win32Application::Run(int32 argc, char **argv)
    {
        String const logFilePath = String::Empty;
			//FileSystem::GetCurrentProcessPath() + m_applicationNameNoWhitespace + "Log.txt";
        ...
        {
            return FatalError(SE_TEXT("Application failed to read settings correctly!"));
        }
        ...
        if (!TryCreateWindow())
        {
            return FatalError(SE_TEXT("Application failed to create window!"));
        }
        ...
        {
            Shutdown();
            return FatalError(SE_TEXT("Application failed to initialize correctly!"));
        }
        else
        {
            m_initialized = true;
        ...
        if (!shutdownResult)
        {
            return FatalError(SE_TEXT("Application failed to shutdown correctly!"));
        }
