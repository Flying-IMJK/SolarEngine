...
namespace SE::Log
{
	...
	public:
		...
		JsonParseException(ErrorCode error, size_t offset) : JsonParseException(error, offset, StringView::Empty)
		{
		}
		...
		JsonParseException(ErrorCode error, size_t offset, const StringView& additionalInfo)
			: Exception(String::Format(SE_TEXT("Parsing Json failed with error code {0} (offset {2}). {1}"), static_cast<int32>(error), GetParseError_En(error), offset), additionalInfo)
		{
		}

		~JsonParseException() override
		{
