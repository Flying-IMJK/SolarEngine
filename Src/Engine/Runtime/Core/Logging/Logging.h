...
namespace SE::Log
{
    ...
    SE_API_RUNTIME void AddEntry(Severity severity, Char const* pCategory, char const* pFilename, int pLineNumber, Char const* pMessage);
    ...
    SE_API_RUNTIME void LogAssert( char const* pFile, int line, Char const* pAssertInfo);

    // Trace to Output Log
    //-------------------------------------------------------------------------

    SE_API_RUNTIME void TraceMessage(Char const* pMessage);

    SE_API_RUNTIME void Notify(NotificationType type, const Char *pFormat);
