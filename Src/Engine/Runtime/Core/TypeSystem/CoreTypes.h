...
namespace SE
{
	...
	class SE_API_RUNTIME CoreTypeRegistry
	{
	public:
	...
	TypeID SE_API_RUNTIME GetCoreTypeID(TypeIDCore coreType);
	TypeIDCore SE_API_RUNTIME GetCoreType(TypeID typeID);
	bool SE_API_RUNTIME IsCoreType(TypeID typeID);

	//-------------------------------------------------------------------------

	template<typename T>
	inline TypeID SE_API_RUNTIME GetCoreTypeID()
	{
		return TypeID();
	}
