...
namespace SE
{
    SE_API_RUNTIME bool ConvertStringToNativeType(TypeID typeID, TypeID templateArgumentTypeID, String const &strValue, void *pValue);
    SE_API_RUNTIME bool ConvertNativeTypeToString(TypeID typeID, TypeID templateArgumentTypeID, void const *pValue, String &strValue);
    SE_API_RUNTIME bool ConvertBinaryToNativeType(TypeID typeID, TypeID templateArgumentTypeID, List<uint8> const &byteArray, void *pValue);
    SE_API_RUNTIME bool ConvertNativeTypeToBinary(TypeID typeID, TypeID templateArgumentTypeID, void const *pValue, List<uint8> &byteArray);
    SE_API_RUNTIME bool ConvertStringToBinary(TypeID typeID, TypeID templateArgumentTypeID, String const &strValue, List<uint8> &byteArray);
    SE_API_RUNTIME bool IsValidStringValueForType(TypeID typeID, TypeID templateArgumentTypeID, String const &strValue);
    ...
    SE_API_RUNTIME void StringToFloatArray(String const &str, int32 const numFloats, float *pFloats);

    // Convert an array of floats into a comma separated string of floats
    SE_API_RUNTIME void FloatArrayToString(float const *pFloats, int32 const numFloats, String &strValue);

    // Convert a comma separated string of ints into an array of ints
    SE_API_RUNTIME void StringToIntArray(String const &str, int32 const numInts, int32 *pInts);

    // Convert an array of ints into a comma separated string of ints
    SE_API_RUNTIME void IntArrayToString(int32 const *pInts, int32 const numInts, String &strValue);
}