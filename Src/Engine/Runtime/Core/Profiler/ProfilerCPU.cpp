...
namespace SE
{
	...
	List<ProfilerCPU::Thread*, InlinedAllocation<64>> ProfilerCPU::Threads;
	...
	void ProfilerCPU::EventBuffer::Extract(List<Event>& data, bool withRemoval)
	{
		data.Clear();
	...
	int32 ProfilerCPU::BeginEvent()
	{
		...
		if (thread == nullptr)
		{
			...
			const auto t = Threading::ThreadRegistry::GetThread(id);
			if (t)
				thread = New<Thread>(t->GetName());
			else if (id == Threading::GetMainThreadID())
				thread = New<Thread>(SE_TEXT("Main"));
			else
				thread = New<Thread>(SE_TEXT("Thread"));
	...
	int32 ProfilerCPU::BeginEvent(const Char* name)
	{
		...
		if (src)
		{
			const auto end = dst + ARRAY_SIZE(e.Name) - 1;
			while (*src && dst != end)
				*dst++ = *src++;
		}
		*dst = 0;
