...
{
    template<>
    class  TTypeCompositeInfo<::SE::Material> final : public TypeCompositeInfo
    {
       static ::SE::Material const* s_pDefaultInstance_8737553902744265008;

    public:
        static void RegisterType()
        {
            s_pDefaultInstance_8737553902744265008 = New<::SE::Material>(nullptr);

            ::SE::Material::s_pTypeInfo = New<TTypeCompositeInfo<::SE::Material>>(s_pDefaultInstance_8737553902744265008);
            Types::RegisterType(::SE::Material::s_pTypeInfo);
        }

        static void UnregisterType()
        {
            Types::UnregisterType(::SE::Material::s_pTypeInfo);
            // Destroy default type instance
            Delete(const_cast<::SE::Material*>(s_pDefaultInstance_8737553902744265008));
            Delete(const_cast<TypeCompositeInfo*>(::SE::Material::s_pTypeInfo));
        }
    ...
    public:
        ...
        {
            // Create type info
            id = TypeID(SE_TEXT("SE::Material"));
            size = sizeof(::SE::Material);
            alignment = alignof(::SE::Material);
            name = SE_TEXT("Material");
            fullName = SE_TEXT("SE::Material");
            ...
            // Add parent info
            // Parent types
            pParentTypeInfo = ::SE::MaterialBase::s_pTypeInfo;
        ...
        {
            auto pMemory = PlatformAllocator::Allocate(sizeof(::SE::Material), alignof(::SE::Material));
            return new (pMemory) ::SE::Material(nullptr);
        }

        virtual void CreateTypeInPlace( IType* pAllocatedMemory ) const override final
        {
            ENGINE_ASSERT( pAllocatedMemory != nullptr );
            new (pAllocatedMemory) ::SE::Material(nullptr);
         }

        virtual IType const *GetDefaultInstance() const override
        {
            return s_pDefaultInstance_8737553902744265008;
        }
        ...
        {
            return AreAllPropertyValuesEqual(pTypeInstance, TTypeCompositeInfo<::SE::Material>::s_pDefaultInstance_8737553902744265008);
        }

        virtual bool IsPropertyValueSetToDefault(IType const *pTypeInstance, uint32 propertyID, int32_t arrayIdx = -1) const override
        {
            return IsPropertyValueEqual(pTypeInstance, TTypeCompositeInfo<::SE::Material>::s_pDefaultInstance_8737553902744265008, propertyID, arrayIdx);
        }
    };

    ::SE::Material const* TTypeCompositeInfo<::SE::Material>::s_pDefaultInstance_8737553902744265008 = nullptr;
}
...
ScriptingTypeInitializer Material::TypeInitializer(
    (BinaryModule*)GetBinaryModuleSERuntime(),
    StringAnsiView("SE.Material", ARRAY_SIZE("SE.Material") - 1),
    sizeof(::SE::Material),
    &MaterialInternal::InitRuntime,
    &ScriptingType::DefaultSpawn, 
    &::SE::MaterialBase::TypeInitializer,
    nullptr,
    nullptr
);
}
