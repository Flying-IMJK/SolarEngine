...
{
    template<>
    class  TTypeCompositeInfo<::SE::Texture> final : public TypeCompositeInfo
    {
       static ::SE::Texture const* s_pDefaultInstance_11464203225968659268;

    public:
        static void RegisterType()
        {
            s_pDefaultInstance_11464203225968659268 = New<::SE::Texture>(nullptr);

            ::SE::Texture::s_pTypeInfo = New<TTypeCompositeInfo<::SE::Texture>>(s_pDefaultInstance_11464203225968659268);
            Types::RegisterType(::SE::Texture::s_pTypeInfo);
        }

        static void UnregisterType()
        {
            Types::UnregisterType(::SE::Texture::s_pTypeInfo);
            // Destroy default type instance
            Delete(const_cast<::SE::Texture*>(s_pDefaultInstance_11464203225968659268));
            Delete(const_cast<TypeCompositeInfo*>(::SE::Texture::s_pTypeInfo));
        }
    ...
    public:
        ...
        {
            // Create type info
            id = TypeID(SE_TEXT("SE::Texture"));
            size = sizeof(::SE::Texture);
            alignment = alignof(::SE::Texture);
            name = SE_TEXT("Texture");
            fullName = SE_TEXT("SE::Texture");
        ...
        {
            auto pMemory = PlatformAllocator::Allocate(sizeof(::SE::Texture), alignof(::SE::Texture));
            return new (pMemory) ::SE::Texture(nullptr);
        }

        virtual void CreateTypeInPlace( IType* pAllocatedMemory ) const override final
        {
            ENGINE_ASSERT( pAllocatedMemory != nullptr );
            new (pAllocatedMemory) ::SE::Texture(nullptr);
         }

        virtual IType const *GetDefaultInstance() const override
        {
            return s_pDefaultInstance_11464203225968659268;
        }
        ...
        {
            return AreAllPropertyValuesEqual(pTypeInstance, TTypeCompositeInfo<::SE::Texture>::s_pDefaultInstance_11464203225968659268);
        }

        virtual bool IsPropertyValueSetToDefault(IType const *pTypeInstance, uint32 propertyID, int32_t arrayIdx = -1) const override
        {
            return IsPropertyValueEqual(pTypeInstance, TTypeCompositeInfo<::SE::Texture>::s_pDefaultInstance_11464203225968659268, propertyID, arrayIdx);
        }
    };

    ::SE::Texture const* TTypeCompositeInfo<::SE::Texture>::s_pDefaultInstance_11464203225968659268 = nullptr;
}
...
ScriptingTypeInitializer Texture::TypeInitializer(
    (BinaryModule*)GetBinaryModuleSERuntime(),
    StringAnsiView("SE.Texture", ARRAY_SIZE("SE.Texture") - 1),
    sizeof(::SE::Texture),
    &TextureInternal::InitRuntime,
