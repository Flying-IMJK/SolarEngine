...
namespace SE
{
	...
	List<ProjectInfo*> ProjectInfo::ProjectsCache;

	void ShowProjectLoadError(const Char* errorMsg, const String& projectRootFolder)
	{
		Platform::Error(String::Format(SE_TEXT("Failed to load project. {0}\nPath: '{1}'"), errorMsg, projectRootFolder));
	}

/*	Float3 GetVector3FromXml(const xml_node& parent, const PUGIXML_CHAR* name, const Float3& defaultValue)
	{
	...
	bool ProjectInfo::SaveProject()
	{
		...
		stream.StartObject();
		{
			...
			if (DefaultScene.IsValid())
			{
				...
				stream.UUID(DefaultScene);
			}

/*			if (DefaultSceneSpawn != Ray(Float3::Zero, Float3::Forward))
			{
	...
	bool ProjectInfo::LoadProject(const String& projectPath)
	{
		...
		{
			ShowProjectLoadError(SE_TEXT("Failed to read file contents."), projectPath);
			return false;
		}
		...
		{
			ShowProjectLoadError(SE_TEXT("Failed to parse project contents. Ensure to have valid Json format."), projectPath);
			return false;
		}
		...
		ProjectFolderPath = FileSystem::GetParentDirectory(projectPath);
		...
		if (versionMember != document.MemberEnd())
		{
			...
			if (version.IsString())
			{
				VersionInfo::Parse(version.GetText(), &Version);
			}
		...
		if (Version.Revision() == 0)
			Version = VersionInfo(Version.Major(), Version.Minor(), Version.Build());
		if (Version.Build() == 0 && Version.Revision() == -1)
			Version = VersionInfo(Version.Major(), Version.Minor());
		...
		if (referencesMember != document.MemberEnd())
		{
			...
			for (int32 i = 0; i < References.Count(); i++)
			{
				...
				if (reference.Name.StartsWith(SE_TEXT("$(EnginePath)")))
				{
					// Relative to engine root
					referencePath = EngineContext::StartupFolder / reference.Name.Substring(14);
				}
				else if (reference.Name.StartsWith(SE_TEXT("$(ProjectPath)")))
				{
				...
				else if (FileSystem::IsRelative(reference.Name))
				{
					...
					referencePath = EngineContext::StartupFolder / reference.Name;
				}
				else
				{
					// Absolute
				...
				FileSystem::PathRemoveRelativeParts(referencePath);

				// Load referenced project
				reference.Project = Load(referencePath);
				if (reference.Project == nullptr)
				{
					LOG_ERROR("Project", "Failed to load referenced project ({0}, from {1})", reference.Name, referencePath);
					return false;
				}
			}
		}
		DefaultScene = JsonTools::GetGuid(document, "DefaultScene");
		// DefaultSceneSpawn = JsonTools::GetRay(document, "DefaultSceneSpawn", Ray(Float3::Zero, Float3::Forward));
		...
		if (minEngineVersionMember != document.MemberEnd())
		{
			...
			{
				VersionInfo::Parse(minEngineVersion.GetText(), &MinEngineVersion);
			}
			else if (minEngineVersionMember->value.IsObject())
			{
				MinEngineVersion = VersionInfo(
		...
		if (Name.Length() == 0)
		{
			ShowProjectLoadError(SE_TEXT("Missing project name."), projectPath);
			return false;
		}
...
/*		xml_document doc;
		...
		if (result.status)
		{
			ShowProjectLoadError(SE_TEXT("Xml file parsing error."), projectPath);
			return true;
		}
		...
		if (!root)
		{
			ShowProjectLoadError(SE_TEXT("Missing Project root node in xml file."), projectPath);
			return true;
		}
		...
		MinEngineVersion = VersionInfo(0, 0, GetIntFromXml(root, PUGIXML_TEXT("MinVersionSupported"), 0));

		// Always reference engine project
		auto& flaxReference = References.AddOne();
		flaxReference.Name = SE_TEXT("$(EnginePath)/Flax.flaxproj");
		flaxReference.Project = Load(EngineContext::StartupFolder / SE_TEXT("Flax.flaxproj"));
		if (!flaxReference.Project)
		{
			ShowProjectLoadError(SE_TEXT("Failed to load Flax Engine project."), projectPath);
			return true;
		}*/
