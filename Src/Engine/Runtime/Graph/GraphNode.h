...
#define GRAPH_NODE_MAKE_TYPE(groupID, typeID) (uint32)((groupID) << 16 | (typeID))
    ...
    public:
        ...
        /// </summary>
        List<GraphBox*, InlinedAllocation<4>> Connections;
    ...
    public:
        ...
        /// <param name="id">Unique box id</param>
        /// <param name="type">Connections type</param>
        GraphBox(void* parent, byte id, const VariantTypeHandle& type)
            : Parent(parent)
            , ID(id)
            , Type(type)
        {
    ...
    public:
        ...
        /// <summary>
        /// List of all node values. List size and value types are constant over time. Only value data can change.
        /// </summary>
        List<Variant, InlinedAllocation<8>> Values;

        /// <summary>
        /// Node boxes cache. List index matches the box ID (for fast O(1) lookups).
        /// </summary>
        List<Box> Boxes;
    ...
    public:
        ...
        void GetBoxes(List<Box*, AllocationType>& result)
        {
            result.Clear();
        ...
        /// <param name="result">The result container.</param>
        template<typename AllocationType = HeapAllocation>
        void GetBoxes(List<const Box*, AllocationType>& result) const
        {
            result.Clear();
            for (int32 i = 0; i < Boxes.Count(); i++)
            {
        ...
        Box* GetBox(int32 id)
        {
            ENGINE_ASSERT(Boxes.HasItems() && Boxes.Count() > id && Boxes[id].ID == id && Boxes[id].Parent == this);
            return &Boxes[id];
        }
