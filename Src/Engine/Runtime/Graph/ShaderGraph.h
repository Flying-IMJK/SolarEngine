...
#define SHADER_GRAPH_MAX_CALL_STACK 100
    ...
    public:
        ...
        ShaderGraphBox(void* parent, byte id, const VariantTypeHandle& type)
            : GraphBox(parent, id, type)
        {
        }
    ...
    public:
        ...
        /// </summary>
        List<Math::BezierCurve<float>> FloatCurves;

        /// <summary>
        /// The Float2 curves used by the graph.
        /// </summary>
        List<Math::BezierCurve<Float2>> Float2Curves;

        /// <summary>
        /// The Float3 curves used by the graph.
        /// </summary>
        List<Math::BezierCurve<Float3>> Float3Curves;

        /// <summary>
        /// The Float4 curves used by the graph.
        /// </summary>
        List<Math::BezierCurve<Float4>> Float4Curves;
    ...
    protected:
        ...
        List<SerializedMaterialParam> _parameters;
        TextWriterUnicode _writer;
        HashSet<String> _includes;
        List<ProcessBoxHandler, FixedAllocation<17>> _perGroupProcessCall;
        List<Node*, FixedAllocation<SHADER_GRAPH_MAX_CALL_STACK>> _callStack;
        List<Graph*, FixedAllocation<32>> _graphStack;
    ...
    protected:
        ...
        SerializedMaterialParam* findParam(const UID& id);

        SerializedMaterialParam findOrAddTexture(const UID& id);
        SerializedMaterialParam findOrAddNormalMap(const UID& id);
        SerializedMaterialParam findOrAddCubeTexture(const UID& id);
