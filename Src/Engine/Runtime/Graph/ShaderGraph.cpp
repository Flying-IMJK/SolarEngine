...
namespace SE
{
    ...
    void ShaderGenerator::ProcessGroupMath(Box* box, Node* node, Value& value)
    {
        ...
        case 5:
        {
            ...
            case 5:
            {
                ...
                if (v2.IsZero())
                {
                    OnError(node, b2, SE_TEXT("Cannot divide by zero!"));
                    v2 = Value::One;
                }
                break;
            }
        ...
        case 11:
        {
            String text = String::Format(SE_TEXT("length({0})"), tryGetValue(node->GetBox(0), Value::Zero).Value);
        ...
        case 24:
        {
            ...
            value = writeFunction3(node, v1, v2, v3, SE_TEXT("clamp"), v1.Type);
            break;
        }
            // Lerp
        case 25:
        {
            ...
            String text = String::Format(SE_TEXT("lerp({0}, {1}, {2})"), a.Value, b.Value, alpha.Value);
        ...
        case 27:
        {
            Box* b1 = node->GetBox(0);
            Value v1 = tryGetValue(b1, Value::Zero);
            value = writeLocal(v1.Type, String(SE_TEXT("-")) + v1.Value, node);
            break;
        }
            // 1 - Value
        case 28:
        ...
        case 29:
        {
            ...
            value = writeLocal(ValueType::Float3, String::Format(SE_TEXT("float3({0}, sqrt(saturate(1.0 - dot({0}.xy, {0}.xy))))"), inXY.Value), node);
            break;
        }
            // Mad
        case 31:
        {
            ...
            String text = String::Format(SE_TEXT("({0}) * ({1}) + ({2})"), v1.Value, Value::Cast(v2, v1.Type).Value, Value::Cast(v3, v1.Type).Value);
        ...
        {
            Value v1 = tryGetValue(node->GetBox(0), Value::Zero);
            String text = String::Format(SE_TEXT("ExtractLargestComponent({0})"), Value::Cast(v1, ValueType::Float3).Value);
        ...
        case 33:
        {
            value = writeFunction1(node, tryGetValue(node->GetBox(0), Value::Zero), SE_TEXT("asin"));
            break;
        }
            // Acosine
        case 34:
        {
            value = writeFunction1(node, tryGetValue(node->GetBox(0), Value::Zero), SE_TEXT("acos"));
            break;
        }
            // Atan
        case 35:
        {
            value = writeFunction1(node, tryGetValue(node->GetBox(0), Value::Zero), SE_TEXT("atan"));
            break;
        }
            // Bias and Scale
        case 36:
        {
            ASSERT(node->Values.Count() == 2 && node->Values[0].Type == VariantType::Types::Float && node->Values[1].Type == VariantType::Types::Float);
            ...
            const String text = String::Format(SE_TEXT("({0} + {1}) * {2}"), input.Value, bias.Value, scale.Value);
        ...
        case 37:
        {
            ...
            const String text = String::Format(SE_TEXT("RotateAboutAxis(float4({0}, {1}), {2}, {3})"), normalizedRotationAxis.Value, shaderGraphValue.Value, graphValue.Value, position.Value);
            _includes.Add(SE_TEXT("./Flax/Math.hlsl"));
            value = writeLocal(ValueType::Float3, text, node);
            break;
        }
        ...
        case 38:
        {
            value = writeFunction1(node, tryGetValue(node->GetBox(0), Value::Zero), SE_TEXT("trunc"));
            break;
        }
            // Frac
        case 39:
        {
            value = writeFunction1(node, tryGetValue(node->GetBox(0), Value::Zero), SE_TEXT("frac"));
            break;
        }
            // Fmod
        case 40:
        {
            Value v1 = tryGetValue(node->GetBox(0), Value::Zero);
            Value v2 = tryGetValue(node->GetBox(1), Value::Zero);
            value = writeFunction2(node, v1, v2, SE_TEXT("fmod"));
            break;
        }
            // Atan2
        case 41:
        {
            Value v1 = tryGetValue(node->GetBox(0), Value::Zero);
            Value v2 = tryGetValue(node->GetBox(1), Value::Zero);
            value = writeFunction2(node, v1, v2, SE_TEXT("atan2"));
            break;
        }
            // Near Equal
        case 42:
        {
            ...
            value = writeLocal(ValueType::Bool, String::Format(SE_TEXT("distance({0},{1}) < {2}"), v1.Value, v2.Value, epsilon.Value), node);
            break;
        }
            // Degrees
        case 43:
        {
            value = writeFunction1(node, tryGetValue(node->GetBox(0), Value::Zero), SE_TEXT("degrees"));
            break;
        }
            // Radians
        case 44:
        {
            value = writeFunction1(node, tryGetValue(node->GetBox(0), Value::Zero), SE_TEXT("radians"));
            break;
        }
            // Remap
        case 48:
        {
            ...
            const auto mapFunc = String::Format(SE_TEXT("{2}.x + ({0} - {1}.x) * ({2}.y - {2}.x) / ({1}.y - {1}.x)"), inVal.Value, rangeA.Value, rangeB.Value);
            value = writeLocal(ValueType::Float, String::Format(SE_TEXT("{2} ? clamp({0}, {1}.x, {1}.y) : {0}"), mapFunc, rangeB.Value, clamp.Value), node);
            break;
        }
            // Rotate Vector
        case 49:
        ...
        case 50:
        {
            ...
            value = writeFunction3(node, v1, v2, v3, SE_TEXT("smoothstep"), v1.Type);
            break;
        }
            // Step
        case 51:
        {
            Value v1 = tryGetValue(node->GetBox(0), 0, Value::Zero);
            Value v2 = tryGetValue(node->GetBox(1), 1, Value::Zero);
            value = writeFunction2(node, v1, v2, SE_TEXT("step"));
            break;
        }
        default:
            break;
    ...
    void ShaderGenerator::ProcessGroupPacking(Box* box, Node* node, Value& value)
    {
        ...
        {
            Value v = tryGetValue(node->GetBox(0), Float4::Zero).AsFloat4();
            value = Value(ValueType::Float2, v.Value + SE_TEXT(".xz"));
            break;
        }
        case 46:
        {
            Value v = tryGetValue(node->GetBox(0), Float4::Zero).AsFloat4();
            value = Value(ValueType::Float2, v.Value + SE_TEXT(".yz"));
            break;
        }
        case 47:
        {
            Value v = tryGetValue(node->GetBox(0), Float4::Zero).AsFloat4();
            value = Value(ValueType::Float2, v.Value + SE_TEXT(".zw"));
            break;
        }
            // Mask XYZ
        case 70:
        ...
        case 100:
        {
            ...
            {
            case 2:
                value = writeLocal(ValueType::Float2, String::Format(SE_TEXT("float2({0}, {1})"), value0.Value, value1.Value), node);
                break;
            case 3:
                value = writeLocal(ValueType::Float3, String::Format(SE_TEXT("float3({0}, {1})"), value0.Value, value1.Value), node);
                break;
            case 4:
                value = writeLocal(ValueType::Float4, String::Format(SE_TEXT("float4({0}, {1})"), value0.Value, value1.Value), node);
                break;
            default:
                value = Value::Zero;
                break;
    ...
    void ShaderGenerator::ProcessGroupTools(Box* box, Node* node, Value& value)
    {
        ...
        case 2:
        {
            ...
            auto dot = writeFunction2(node, input, luminanceFactors, SE_TEXT("dot"), ValueType::Float);
            value = writeFunction3(node, input, dot, scale, SE_TEXT("lerp"), ValueType::Float3);
            break;
        }
            // Color Gradient
        case 10:
        {
            ...
            case 2:
                ...
                value = writeLocal(ValueType::Float4, String::Format(
                                       SE_TEXT("lerp({0}, {1}, saturate(({2} - {3}) / ({4} - {3})))"),
                                       prevColor.Value,
                                       curColor.Value,
                                       time.Value,
                                       prevTime.Value,
            ...
            GraphUtilities::SampleCurve(_writer, _graphStack.Peek()->curves[node->Data.Curve.CurveIndex], time.Value, value.Value); \
            break; \
            }
            ...
            /*case 16:
        {
            ...
            auto asset = Assets.LoadAsync<GameplayGlobals>((UID)node->Values[0]);
            if (!asset || asset->WaitForLoaded())
            {
                OnError(node, box, SE_TEXT("Failed to load Gameplay Global asset."));
                value = Value::Zero;
                break;
            }
            ...
            {
                OnError(node, box, SE_TEXT("Missing Gameplay Global variable."));
                value = Value::Zero;
                break;
            }
        ...
        case 17:
        {
            ...
            List<Value, FixedAllocation<32>> values;
...
_writer.Write(SE_TEXT("#ifdef " defineName "\n\t{0} = {1};\n#endif\n"), value.Value, values[boxIndex].Value)
...
#undef PLATFORM_CASE
        ...
        case 34:
        {
            ...
            {
            case 30:
                format = SE_TEXT("PerlinNoise({0})");
                break;
            case 31:
                format = SE_TEXT("SimplexNoise({0})");
                break;
            case 32:
    ...
    void ShaderGenerator::ProcessGroupBoolean(Box* box, Node* node, Value& value)
    {
        ...
        case 1:
        {
            ...
            // Perform operation
            value = writeLocal(ValueType::Bool, String::Format(SE_TEXT("!{0}"), a.Value), node);
            break;
        }
            // AND, OR, XOR, NOR, NAND
        case 2:
    ...
    void ShaderGenerator::ProcessGroupBitwise(Box* box, Node* node, Value& value)
    {
        ...
        {
            // Get A value
            const Value a = tryGetValue(node->GetBox(0), Value(ValueType::Int, SE_TEXT("0"))).AsInt();

            // Perform operation
            value = writeLocal(ValueType::Int, String::Format(SE_TEXT("!{0}"), a.Value), node);
            break;
        }
            // AND, OR, XOR
        case 2:
    ...
    void ShaderGenerator::ProcessGroupComparisons(Box* box, Node* node, Value& value)
    {
        ...
        case 7:
        {
            ...
            value = writeLocal(onTrue.Type, String::Format(SE_TEXT("{0} ? {1} : {2}"), condition.Value, onTrue.Value, onFalse.Value), node);
            break;
        }
        }
    }
    ...
    ShaderGenerator::Value ShaderGenerator::eatBox(Node* caller, Box* box)
    {
        ...
        {
            OnError(caller, box, SE_TEXT("Graph is looped or too deep!"));
            return Value::Zero;
        }
        ...
        if (value.IsInvalid())
        {
            OnError(parentNode, box, SE_TEXT("Unknown box to resolve."));
        }
    ...
    ShaderGenerator::Value ShaderGenerator::writeLocal(ValueType type, Node* caller, const String& name)
    {
        ...
        {
        case ValueType::Bool:
            typeName = SE_TEXT("bool");
            break;
        case ValueType::Int:
            typeName = SE_TEXT("int");
            break;
        case ValueType::Uint:
            typeName = SE_TEXT("uint");
            break;
        case ValueType::Float:
            typeName = SE_TEXT("float");
            break;
        case ValueType::Float2:
            typeName = SE_TEXT("float2");
            break;
        case ValueType::Float3:
            typeName = SE_TEXT("float3");
            break;
        case ValueType::Float4:
        case ValueType::Color:
            typeName = SE_TEXT("float4");
            break;
        case ValueType::Object:
            typeName = SE_TEXT("Texture2D");
            break;
        case ValueType::Void:
            typeName = SE_TEXT("Material");
            break;
        default:
            OnError(caller, nullptr, *String::Format(SE_TEXT("Unsupported value type: {0}"), VariantTypeHandle(type).ToString()));
    ...
    ShaderGenerator::Value ShaderGenerator::writeOperation2(Node* caller, const Value& valueA, const Value& valueB, Char op1)
    {
        ...
        const String value = String::Format(SE_TEXT("{0} {1} {2}"), valueA.Value, op1Str, Value::Cast(valueB, valueA.Type).Value);
    ...
    {
        const String value = String::Format(SE_TEXT("{0}({1})"), function, valueA.Value);
        return writeLocal(valueA.Type, value, caller);
    }
    ...
    {
        const String value = String::Format(SE_TEXT("{0}({1}, {2})"), function, valueA.Value, Value::Cast(valueB, valueA.Type).Value);
        return writeLocal(valueA.Type, value, caller);
    }
    ...
    {
        const String value = String::Format(SE_TEXT("{0}({1}, {2}, {3})"), function, valueA.Value, Value::Cast(valueB, valueA.Type).Value, Value::Cast(valueC, valueA.Type).Value);
        return writeLocal(resultType, value, caller);
    }
    ...
    SerializedMaterialParam* ShaderGenerator::findParam(const UID& id)
    {
    ...
    SerializedMaterialParam ShaderGenerator::findOrAddTexture(const UID& id)
    {
        // Find
        for (int32 i = 0; i < _parameters.Count(); i++)
        {
        ...
        param.Override = true;
        param.Name = SE_TEXT("Texture");
        param.ShaderName = getParamName(_parameters.Count());
        param.AsGuid = id;
        param.ID = UID(_parameters.Count(), 0, 0, 1); // Assign temporary id
        return param;
    }

    SerializedMaterialParam ShaderGenerator::findOrAddNormalMap(const UID& id)
    {
        // Find
        for (int32 i = 0; i < _parameters.Count(); i++)
        {
        ...
        param.IsPublic = false;
        param.Override = true;
        param.Name = SE_TEXT("Normal Map");
        param.ShaderName = getParamName(_parameters.Count());
        param.AsGuid = id;
        param.ID = UID(_parameters.Count(), 0, 0, 2); // Assign temporary id
        return param;
    }

    SerializedMaterialParam ShaderGenerator::findOrAddCubeTexture(const UID& id)
    {
        // Find
        for (int32 i = 0; i < _parameters.Count(); i++)
        {
        ...
        param.IsPublic = false;
        param.Override = true;
        param.Name = SE_TEXT("Cube Texture");
        param.ShaderName = getParamName(_parameters.Count());
        param.AsGuid = id;
        param.ID = UID(_parameters.Count(), 0, 0, 3); // Assign temporary id
        return param;
    }
    ...
    SerializedMaterialParam ShaderGenerator::findOrAddSceneTexture(MaterialSceneTextures type)
    {
        ...
        param.IsPublic = false;
        param.Override = true;
        param.Name = SE_TEXT("Scene Texture");
        param.ShaderName = getParamName(_parameters.Count());
    ...
    SerializedMaterialParam& ShaderGenerator::findOrAddTextureGroupSampler(int32 index)
    {
        ...
        param.IsPublic = false;
        param.Override = true;
        param.Name = SE_TEXT("Texture Group Sampler");
        param.ShaderName = getParamName(_parameters.Count());
    ...
    {
        return SE_TEXT("local") + StringUtils::ToString(index);
    }

    String ShaderGenerator::getParamName(int32 index)
    {
        return SE_TEXT("In") + StringUtils::ToString(index);
    }
} // SE