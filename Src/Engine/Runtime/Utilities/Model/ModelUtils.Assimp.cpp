...
namespace SE
{
    class AssimpLogStream final : public LogStream
    {
    public:
        AssimpLogStream()
        {
        ...
        ~AssimpLogStream() override
        {
            DefaultLogger::get()->detachStream(this);
            DefaultLogger::kill();
        }
    ...
    struct AssimpImporterData
    {
        ...
        const ModelUtils::Options& Options;

        List<AssimpNode> Nodes;
        List<AssimpBone> Bones;
        Dictionary<int32, List<int32>> MeshIndexToNodeIndex;

        AssimpImporterData(const StringView& path, const ModelUtils::Options& options)
            : Path(path)
            , Options(options)
        {
        }
    ...
    void ProcessNodes(AssimpImporterData& data, aiNode* aNode, int32 parentIndex)
    {
        ...
        else
        {
            ...
            if (node.LodIndex == 0)
            {
                node.LodIndex = ModelUtils::DetectLodIndex(node.Name);
            }
    ...
    bool ProcessMesh(ModelData& result, AssimpImporterData& data, const aiMesh* aMesh, MeshData& mesh, String& errorMsg)
    {
        ...
        for (unsigned faceIndex = 0, i = 0; faceIndex < aMesh->mNumFaces; faceIndex++)
        {
            ...
            {
                errorMsg = SE_TEXT("All faces in a mesh must be trangles!");
                return false;
            }
        ...
        if (data.Options.CalculateNormals || !aMesh->mNormals)
        {
            ...
            {
                errorMsg = SE_TEXT("Failed to generate normals.");
                return false;
            }
        }
        ...
        else if (data.Options.LightmapUVsSource == ModelLightmapUVsSource::Generate)
        {
            ...
            {
                LOG_ERROR("Resource", "Failed to generate lightmap uvs");
            }
        }
        else
        {
            ...
            {
                LOG_WARNING("Resource", "Cannot import result lightmap uvs. Missing texcoords channel {0}.", inputChannelIndex);
            }
        }
        */
        ...
        if (aMesh->mNumBones > 0 && aMesh->mBones && data.Options.ImportTypes.IsFlag(ImportDataTypes::Skeleton))
        {
            ...
            for (unsigned boneId = 0; boneId < aMesh->mNumBones; boneId++)
            {
                ...
                if (nodeIndex == -1)
                {
                    LOG_WARNING("Resource", "Invalid mesh bone linkage. Mesh: {0}, bone: {1}. Skipping...", mesh.Name, boneName);
                    continue;
                }
        ...
        if (aMesh->mNumAnimMeshes > 0 && data.Options.ImportTypes.IsFlag(ImportDataTypes::Skeleton) && data.Options.ImportBlendShapes)
        {
            ...
            for (unsigned int animMeshIndex = 0; animMeshIndex < aMesh->mNumAnimMeshes; animMeshIndex++)
            {
                ...
                if (blendShapeData.Name.IsEmpty())
                    blendShapeData.Name = mesh.Name + SE_TEXT("_blend_shape_") + StringUtils::ToString(animMeshIndex);
    ...
    bool ImportTexture(ModelData& result, AssimpImporterData& data, aiString& aFilename, int32& textureIndex, TextureEntry::TypeHint type)
    {
        ...
        String path;
        if (!ModelUtils::FindTexture(data.Path, filename, path))
            return false;
    ...
    bool ImportMaterialTexture(ModelData& result, AssimpImporterData& data, const aiMaterial* aMaterial, aiTextureType aTextureType, int32& textureIndex, TextureEntry::TypeHint type)
    {
        ...
        if (aMaterial->GetTexture(aTextureType, 0, &aFilename, nullptr, nullptr, nullptr, nullptr) == AI_SUCCESS)
        {
            ...
            if (filename.StartsWith(SE_TEXT(AI_EMBEDDED_TEXNAME_PREFIX)))
            {
                const aiTexture* aTex = data.Scene->GetEmbeddedTexture(aFilename.C_Str());
                const StringView texIndexName(filename.Get() + (ARRAY_SIZE(AI_EMBEDDED_TEXNAME_PREFIX) - 1));
                uint32 texIndex;
                ...
                if (aTex && aTex->mHeight == 0 && aTex->mWidth > 0)
                {
                    ...
                    filename = String::Format(SE_TEXT("{0}_tex_{1}.{2}"), FileSystem::GetFileNameWithoutExtension(data.Path), texIndexName, String(aTex->achFormatHint));
                    File::WriteAllBytes(String(FileSystem::GetDirectoryName(data.Path)) / filename, (const byte*)aTex->pcData, (int32)aTex->mWidth);
                }
            }
            ...
            if (data.Options.ImportTypes.IsFlag(ImportDataTypes::Materials))
            {
                ...
                if (data.Options.ImportTypes.IsFlag(ImportDataTypes::Textures))
                {
                    ...
                    if (materialSlot.Diffuse.TextureIndex != -1)
                    {
                        ...
                        // TODO
                        materialSlot.Diffuse.HasAlphaMask = false;// TextureTool::HasAlpha(result.Textures[materialSlot.Diffuse.TextureIndex].FilePath);
    ...
    bool ModelUtils::ImportDataAssimp(const String& path, ModelData& data, Options& options, String& errorMsg)
    {
        ...
        {
            LOG_WARNING("Resource", "{0}", String(context.AssimpImporter.GetErrorString()));
            LOG_WARNING("Resource", "{0}", String(path));
            LOG_WARNING("Resource", "{0}", StringUtils::ToString(flags));
        ...
        {
            LOG_WARNING("Resource", "Failed to import materials.");
            return false;
        }
