...
namespace tracy
{
	...
	public:
		...
		void Text( const SE::Char* txt, size_t size )
		{
			assert( size < (std::numeric_limits<uint16_t>::max)() );
			if( !m_active ) return;
#ifdef TRACY_ON_DEMAND
		...
		tracy_force_inline void Name( const SE::Char* txt, size_t size )
		{
			assert( size < (std::numeric_limits<uint16_t>::max)() );
			if( !m_active ) return;
#ifdef TRACY_ON_DEMAND
	...
	inline bool ScopedZone::Begin(const SourceLocationData* srcloc)
	{
#ifdef TRACY_ON_DEMAND
