...
#include "Runtime/DebugViews/DebugView.h"
...
namespace SE
{
    ...
    {
        ENGINE_ASSERT( !pathString.empty() );
        size_t const categoryNameLength = pathString.length();

        // Validation
        ENGINE_ASSERT( categoryNameLength < 256 );
        if ( categoryNameLength > 0 )
        {
            for ( auto i = 0; i < categoryNameLength - 1; i++ )
            {
                ENGINE_ASSERT( isalnum( pathString[i] ) || pathString[i] == ' ' || pathString[i] == '/' );
            }
        }
    ...
    {
        ENGINE_ASSERT( !TryFindMenu( pDebugView ) );
    ...
    {
        ENGINE_ASSERT( pDebugView != nullptr );
        ENGINE_ASSERT( pDebugView->HasMenu() );
    ...
    EngineDebugUI::Menu& EngineDebugUI::Menu::FindOrAddMenu( MenuPath const& path )
    {
        ...
        while ( true )
        {
            ...
            if ( !childMenuFound )
            {
                ...
                VectorSort(pCurrentMenu->m_childMenus, menuSortPredicate );
    ...
    void EngineDebugUI::Menu::Draw( EntityWorldUpdateContext const& context )
    {
        for ( auto& childMenu : m_childMenus )
        {
            ENGINE_ASSERT( !childMenu.IsEmpty() );
    ...
    void EngineDebugUI::Initialize( UpdateContext const& context)
    {
        ...
        {
            auto pTypeRegistry = context.GetSystem<TypeRegistry>();
            List<TypeInfo const*> debugViewTypeInfos = pTypeRegistry->GetAllDerivedTypes( DebugView::GetStaticTypeID(), false, false, true );
    ...
    void EngineDebugUI::EndFrame( UpdateContext const& context )
    {
        ...
        ENGINE_ASSERT( updateStage == UpdateStage::FrameEnd );
    ...
    void EngineDebugUI::DrawMenu( UpdateContext const& context )
    {
        ...
        TInlineString<10> const warningsStr( TInlineString<10>::CtorSprintf(), ICON_ALERT" %d", Log::System::GetNumWarnings() );
        TInlineString<10> const errorsStr( TInlineString<10>::CtorSprintf(), ICON_CLOSE_CIRCLE" %d", Log::System::GetNumErrors() );
        ...
        bool const drawDebugMenu = ImGui::BeginMenu( ICON_CLOCK_FAST );
        ImGui::PopStyleColor();
        if ( drawDebugMenu )
        {
            SGUI::TextSeparator( ICON_CLOCK" Time Controls" );
            {
                ...
                {
                    if ( ImGui::Button( ICON_PLAY"##ResumeWorld", buttonSize ) )
                    {
                        ToggleWorldPause();
                    }
                    SGUI::ItemTooltip( "Resume" );
                }
                else
                {
                    if ( ImGui::Button( ICON_PAUSE"##PauseWorld", buttonSize ) )
                    {
                        ToggleWorldPause();
                    }
                    SGUI::ItemTooltip( "Pause" );
                }

                // Step
                ImGui::SameLine();
                ImGui::BeginDisabled( !m_pGameWorld->IsPaused() );
                if ( ImGui::Button( ICON_ARROW_RIGHT_BOLD"##StepFrame", buttonSize ) )
                {
                    RequestWorldTimeStep();
                }
                SGUI::ItemTooltip( "Step Frame" );
                ImGui::EndDisabled();
                ...
                SGUI::ItemTooltip( "Time Scale" );

                // Reset
                ImGui::SameLine();
                if ( ImGui::Button( ICON_UNDO"##ResetTimeScale", buttonSize ) )
                {
                    ResetWorldTimeScale();
                }
                SGUI::ItemTooltip( "Reset TimeScale" );
            }

            ImGui::EndMenu();
        }
        SGUI::ItemTooltip( "World Time Controls" );

        // Player Debug Options
        //-------------------------------------------------------------------------

        SGUI::SameLineSeparator( gapSize );

        if ( ImGui::BeginMenu( ICON_CONTROLLER_CLASSIC "##PlayerDebugOptions") )
        {
            DrawPlayerDebugOptionsMenu( context );
            ImGui::EndMenu();
        }
        SGUI::ItemTooltip( "Player Debug Options" );

        SGUI::SameLineSeparator( gapSize );

        SystemDebugView::DrawFrameLimiterCombo( const_cast<UpdateContext&>( context ) );
        SGUI::ItemTooltip( "Frame Rate Limiter" );
        ...
        if ( SGUI::FlatButton( warningsStr.c_str() ) )
        {
            if ( auto pSystemDebugView = FindDebugView<SystemDebugView>() )
            {
                pSystemDebugView->m_windows[0].m_isOpen = true;
        ...
        ImGui::PopStyleColor();
        SGUI::ItemTooltip( "Warnings" );

        SGUI::SameLineSeparator( gapSize );

        ImGui::PushStyleColor( ImGuiCol_Text, Colors::Red );
        if ( SGUI::FlatButton( errorsStr.c_str() ) )
        {
            if ( auto pSystemDebugView = FindDebugView<SystemDebugView>() )
            {
                pSystemDebugView->m_windows[0].m_isOpen = true;
    ...
    {
        ENGINE_ASSERT( context.GetUpdateStage() == UpdateStage::FrameEnd );
        ...
        if ( m_debugOverlayEnabled )
        {
            ImVec2 const guidePosition = ImGui::GetWindowPos() + ImGui::GetWindowContentRegionMax() - ( SGUI::OrientationGuide::GetSize() / 2 );
            SGUI::OrientationGuide::Draw( guidePosition, *pViewport );
        }
    }

    void EngineDebugUI::HotReload_UnloadResources( List<ResourceRequesterID> const& usersToReload, List<ResID> const& resourcesToBeReloaded )
    {
        for ( auto pDebugView : m_debugViews )
        {
    ...
    void EngineDebugUI::DrawPlayerDebugOptionsMenu( UpdateContext const& context )
    {
        ...
        if ( ImGui::Checkbox( ICON_MICROSOFT_XBOX_CONTROLLER " Enable Player Controller", &isControllerEnabled ) )
        {
        ...
        if ( ImGui::Checkbox( ICON_CCTV " Enable Debug Camera", &isDebugCameraEnabled ) )
        {
    ...
    void EngineDebugUI::HandleUserInput( UpdateContext const& context )
    {
        ...
        ENGINE_ASSERT( pInputSystem != nullptr );
