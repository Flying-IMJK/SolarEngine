...
#include "Core/Platform/Win32/Application_Win32.h"
...
namespace SE::SGUI
{
    namespace Platform
    {
        ...
        struct ImGui_ImplWin32_ViewportData
        {
            ...
            ImGui_ImplWin32_ViewportData() { window = NULL; HwndOwned = false;  DwStyle = DwExStyle = 0; }
            ~ImGui_ImplWin32_ViewportData() { ENGINE_ASSERT( window == NULL ); }
        };
        ...
        static void ImGui_Impl_CreateWindow( ImGuiViewport* viewport )
        {
            ...
            if ( viewport->ParentViewportId != 0 )
			{
				...
				{
					parent_window = (Window*)parent_viewport->PlatformHandle;
				}
			}

            // Create window
			CreateWindowSettings settings;
			settings.Position = viewport->Pos;
			settings.Size = viewport->Size;
			settings.Parent = parent_window;
...
*/
            ...
            //auto const errorMessage = SE::Platform::Win32::GetLastErrorMessage();
            ENGINE_ASSERT( vd->window != 0 );
        }
        ...
        static void ImGui_Impl_DestroyWindow( ImGuiViewport* viewport )
        {
            ...
            {
                if ( ::GetCapture() == vd->window->GetHWND() )
                {
                    // Transfer capture so if we started dragging from a window that later disappears, we'll still receive the MOUSEUP event.
                    ::ReleaseCapture();
                    ::SetCapture( vd->window->GetHWND() );
                }
                if ( vd->window && vd->HwndOwned )
				{
					Delete(vd->window);// ::DestroyWindow( vd->Hwnd );
				}
                vd->window = nullptr;
                Delete( vd );
            }
        ...
        static void ImGui_Impl_ShowWindow( ImGuiViewport* viewport )
        {
			...
			{
				vd->window->GetSettings().ActivateWhenFirstShown = false;
			}
			else
			{
				vd->window->GetSettings().ActivateWhenFirstShown = true;
			}

			vd->window->Show();
        }

        static void ImGui_Impl_UpdateWindow( ImGuiViewport* viewport )
        {
            ...
            if ( vd->DwStyle != new_style || vd->DwExStyle != new_ex_style )
            {
                ...
                ::SetWindowLong( vd->window->GetHWND(), GWL_STYLE, vd->DwStyle );
                ::SetWindowLong( vd->window->GetHWND(), GWL_EXSTYLE, vd->DwExStyle );
                RECT rect = { (LONG)viewport->Pos.x, (LONG)viewport->Pos.y, (LONG)( viewport->Pos.x + viewport->Size.x ), (LONG)( viewport->Pos.y + viewport->Size.y ) };
                ::AdjustWindowRectEx( &rect, vd->DwStyle, FALSE, vd->DwExStyle ); // Client to Screen
                ::SetWindowPos( vd->window->GetHWND(), insert_after, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, swp_flag | SWP_NOACTIVATE | SWP_FRAMECHANGED );
                ::ShowWindow( vd->window->GetHWND(), SW_SHOWNA ); // This is necessary when we alter the style
        ...
        static ImVec2 ImGui_Impl_GetWindowPos( ImGuiViewport* viewport )
        {
            ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*)viewport->PlatformUserData;
            IM_ASSERT( vd->window != 0 );
            return vd->window->GetClientPosition();
        }
        ...
        static ImVec2 ImGui_Impl_GetWindowSize( ImGuiViewport* viewport )
        {
            ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*)viewport->PlatformUserData;
            IM_ASSERT( vd->window != 0 );
            return vd->window->GetClientSize();
        }
...
#undef IsMinimized
        ...
        {
            // Strip non-ascii characters from title
            StringAnsi str( title );
			str.LastTrim();
			str.FirstTrim();
			
            ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*) viewport->PlatformUserData;
            IM_ASSERT( vd->window != 0 );
			vd->window->SetTitle(str.ToString());
        }
        ...
        static void ImGui_Impl_SetWindowAlpha( ImGuiViewport* viewport, float alpha )
        {
			...
			vd->window->SetOpacity(alpha);
        }

        static float ImGui_Impl_GetWindowDpiScale( ImGuiViewport* viewport )
        {
            ImGui_ImplWin32_ViewportData* vd = (ImGui_ImplWin32_ViewportData*)viewport->PlatformUserData;
            IM_ASSERT( vd->window != 0 );
        ...
        static void ImGui_ImplWin32_UpdateMouseData()
        {
            ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
            ImGuiIO& io = ImGui::GetIO();
            IM_ASSERT( bd->window != 0 );

            POINT mouse_screen_pos;
            bool has_mouse_screen_pos = ::GetCursorPos( &mouse_screen_pos ) != 0;

            HWND focused_window = ::GetForegroundWindow();
            const bool is_app_focused = ( focused_window && ( focused_window == bd->window->GetHWND() || ::IsChild( focused_window, bd->window->GetHWND() ) || ImGui::FindViewportByPlatformHandle( (void*)focused_window ) ) );
            if ( is_app_focused )
            {
                ...
                if ( !io.WantSetMousePos && bd->MouseTrackedArea == 0 && has_mouse_screen_pos )
                {
                    ...
                    if ( !( io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable ) )
                        ::ScreenToClient( bd->window->GetHWND(), &mouse_pos );
...
/*            if ( pBackendData->m_pEngineInputSystem != nullptr )
        ...
        intptr WindowMessageProcessor( HWND hWnd, uint32 message, uintptr wParam, intptr lParam )
        {
    ...
    void GUISystem::InitializePlatform()
    {
        ImGuiIO& io = ImGui::GetIO();

        //-------------------------------------------------------------------------

        auto pBackendData = New<Platform::ImGui_ImplWin32_Data>();

        pBackendData->window = Window::mainWindow;
...
//        pBackendData->m_pEngineInputSystem = m_pInputSystem;
        ...
        pMainViewport->PlatformHandle = pMainViewport->PlatformHandleRaw = (void*) pBackendData->window;
        ...
        if ( io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable )
        {
            ...
            platform_io.Platform_CreateWindow = Platform::ImGui_Impl_CreateWindow;
            platform_io.Platform_DestroyWindow = Platform::ImGui_Impl_DestroyWindow;
            platform_io.Platform_ShowWindow = Platform::ImGui_Impl_ShowWindow;
            platform_io.Platform_SetWindowPos = Platform::ImGui_Impl_SetWindowPos;
            platform_io.Platform_GetWindowPos = Platform::ImGui_Impl_GetWindowPos;
            platform_io.Platform_SetWindowSize = Platform::ImGui_Impl_SetWindowSize;
            platform_io.Platform_GetWindowSize = Platform::ImGui_Impl_GetWindowSize;
            platform_io.Platform_SetWindowFocus = Platform::ImGui_Impl_SetWindowFocus;
            platform_io.Platform_GetWindowFocus = Platform::ImGui_Impl_GetWindowFocus;
            platform_io.Platform_GetWindowMinimized = Platform::ImGui_Impl_GetWindowMinimized;
            platform_io.Platform_SetWindowTitle = Platform::ImGui_ImplWin32_SetWindowTitle;
            platform_io.Platform_SetWindowAlpha = Platform::ImGui_Impl_SetWindowAlpha;
            platform_io.Platform_UpdateWindow = Platform::ImGui_Impl_UpdateWindow;

            // Get the icon from the main application - cant believe that there is no way to get this via the win32 API
            HICON childWindowIcon = nullptr;
            auto pApplication = reinterpret_cast<Win32Application*>( ::GetWindowLongPtrW( pBackendData->window, GWLP_USERDATA ) );
            if ( pApplication != nullptr )
            {
                childWindowIcon = pApplication->GetIcon();
            }
...
//            pBackendData->m_imguiWindowClass.hIcon = childWindowIcon;
            ...
            auto pVD = New<Platform::ImGui_ImplWin32_ViewportData>();
            pVD->window = pBackendData->window;
            pVD->HwndOwned = false;
            pMainViewport->PlatformUserData = pVD;
        }
    }

    void GUISystem::ShutdownPlatform()
    {
