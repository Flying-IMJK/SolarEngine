...
namespace SE::GUI
{
    ...
    public:
        ...
        ComboWithFilterWidget( TBFlags<Flags> flags = TBFlags<Flags>() ) : m_flags( flags ) {}
        ...
        inline bool HasValidSelection() const { return m_selectedIdx != -1; }

        // Get the currently selected option
        Option const* GetSelectedOption() const { return ( m_selectedIdx == -1 ) ? nullptr : &m_filteredOptions[m_selectedIdx]; }
    ...
    protected:
        ...
        List<Option>      m_filteredOptions;
        TBFlags<Flags>    m_flags = TBFlags<Flags>();
    ...
    bool ComboWithFilterWidget<T>::DrawAndUpdate( float width, float minDropDownWidth, float dropDownHeightInItems )
    {
        ...
        if ( ImGui::BeginChild( "FilterLayout", ImVec2( width, ImGui::GetFrameHeight() ), ImGuiWindowFlags_AlwaysUseWindowPadding, ImGuiWindowFlags_NoScrollbar ) )
        {
            ...
            if ( m_isComboOpen )
            {
                if ( m_filterWidget.UpdateAndDraw( -1, SGUI::FilterWidget::Flags::TakeInitialFocus ) )
                {
                    ApplyFilterToOptions();
                }
    ...
    template<typename T>
    void SE::SGUI::ComboWithFilterWidget<T>::DrawPreview() const
    {
        ImGui::Text( ( m_selectedIdx == -1 ) ? "" : m_filteredOptions[m_selectedIdx].m_label.c_str() );
    }

    template<typename T>
    bool SE::SGUI::ComboWithFilterWidget<T>::DrawOption( Option const& option ) const
    {
        if ( option.m_isSeparator )
        {
            ...
            {
                SGUI::TextSeparator( option.m_label.c_str() );
            }
    ...
    template<typename T>
    void SE::SGUI::ComboWithFilterWidget<T>::SetSelectedOption( T const& selectedItem )
    {
        m_selectedIdx = -1;
        GenerateOptions();
        ...
        if ( selectedItem.IsValid() )
        {
            ...
            {
                if ( !m_filteredOptions[i].m_isSeparator && m_filteredOptions[i].value == selectedItem )
                {
                    m_selectedIdx = i;
                    break;
                }
